...
int OnInit()
{
   ...
   if(trade_symbol == "")
   {
      Print("EVE Momentum v2.01: could not resolve a trading symbol.");
      return INIT_FAILED;
   }
   if(!SymbolSelect(trade_symbol, true))
   {
      Print("EVE Momentum v2.01: cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE Momentum v2.01: indicator handle creation failed. Error ", GetLastError());
      return INIT_FAILED;
   }
...
bool PostJson(string endpoint, string json)
{
   ...
   {
      PrintFormat("EVE Momentum v2.01 POST %s failed. HTTP=%d MQL=%d response=%s", endpoint, status, GetLastError(), CharArrayToString(result));
      return false;
   }
   return true;
}
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX+"TITLE", 10, 18, "EVE MOMENTUM BURST v2.01 - MOVING STRADDLE", 12, clrWhite);
