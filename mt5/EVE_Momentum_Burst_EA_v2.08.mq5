...
input ulong  InpMagicNumber                    = 2207202608;
input string InpOrderComment                   = "EVE-MOMENTUM-V2.08";

input group "One-time legacy isolation"
input bool   InpDeleteLegacyPendingOrders       = true;                // Deletes v2.06/v2.07 EVE pending orders before v2.08 starts
input ulong  InpLegacyMagicNumber               = 2207202606;          // Previous v2.06/v2.07 EVE magic number
...
input int    InpReversalArmDelaySeconds         = 0;                   // Legacy input; unused in v2.08
input double InpReversalArmMinimumProfitMoney   = 0.0;                 // Legacy input; unused in v2.08
...
input double InpAddOnlyAfterBasketProfit        = 0.0;                 // Legacy input; unused in v2.08
input bool   InpNeverAddToLosingBasket          = false;               // Pending ladder is price-triggered, not P/L-gated
input double InpNewestLegProfitBeforeAddMoney   = 0.0;                 // Legacy input; unused in v2.08
...
int OnInit()
{
   ...
   if(trade_symbol == "")
   {
      Print("EVE Momentum v2.08: could not resolve a trading symbol.");
      return INIT_FAILED;
   }
   if(!SymbolSelect(trade_symbol, true))
   {
      Print("EVE Momentum v2.08: cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE Momentum v2.08: indicator handle creation failed. Error ", GetLastError());
      return INIT_FAILED;
   }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result)
{
   ...
   if(entry == DEAL_ENTRY_IN || entry == DEAL_ENTRY_INOUT)
   {
      ...
      if(first_leg)
      {
         ...
         campaign_phase = CAMPAIGN_OCO_CANCELLING; // v2.08: first of two same-side confirmations
         basket_entry_atr = CurrentOrderAtr();
...
bool DetectExecutionIntegrityBreach(const MqlTick &tick)
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      {
         if(sl >= entry - tolerance)
         {
            FlagExecutionIntegrityBreach(StringFormat("BUY position %I64u has invalid SL geometry: entry %.2f / SL %.2f", ticket, entry, sl), identifier);
            return true;
         }
      ...
      else
      {
         if(sl <= entry + tolerance)
         {
            FlagExecutionIntegrityBreach(StringFormat("SELL position %I64u has invalid SL geometry: entry %.2f / SL %.2f", ticket, entry, sl), identifier);
            return true;
         }
...
{
   // v2.08 never adds with market orders. This helper remains only for backward compatibility.
   return false;
}
...
{
   return StringFind(comment, "EVE26-") >= 0 || StringFind(comment, "EVE-MOMENTUM-V2.06") >= 0 || StringFind(comment, "EVE-MOMENTUM-V2.07") >= 0 ||
...
bool HandleLegacyPendingCleanup()
{
   ...
   last_event = "v2.08 is deleting v2.06/v2.07 pending orders before starting its own bracket";
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      delete_wait_ticket = ticket;
      delete_wait_started_ms = GetTickCount64();
      SendOrderActivity("LEGACY-CANCEL-REQUESTED", "LEGACY", type, ticket, volume, price, "v2.08 version isolation cleanup");
      return true;
   }
   return true;
}
...
void ManageBasket()
{
   ...
   if(InpBlockOnLegacyOpenPosition && CountLegacyEvePositions() > 0)
   {
      ...
      pending_sync_state = "CLOSE OLD V2.06/V2.07 POSITION BEFORE V2.08";
      last_event = "v2.08 is isolated and will not overlap an open position from the previous EVE magic number";
      CancelPendingOrders();
      return;
   }
...
void MaintainMovingStraddle()
{
   ...
   // v2.08 never removes both sides simply because a new candle started.
...
{
   // Legacy v2.04 hook intentionally disabled. v2.08 uses only the two-trigger
...
bool PlacePendingOrder(ENUM_ORDER_TYPE type, string role, double volume, double price, double atr_value, double protect_anchor_price, string reason)
{
   ...
   string comment = "EVE28-LAD";
   if(role == "INITIAL" || role == "ENTRY-BRACKET") comment = "EVE28-INIT";
   else if(role == "CONFIRMATION" || role == "REVERSAL") comment = "EVE28-CONF";
...
void TryAddRollingLeg()
{
   ...
   // All v2.08 additions come from broker-side pending stop orders.
}
...
void ContinuePendingClose()
{
   ...
   // v2.08 quarantines the campaign first: pending entries are cancelled before
...
void FinaliseBasketClose(string fallback_reason)
{
   ...
   campaign_cooldown_until = 0; // v2.08 restarts immediately; no strategy cooldown.
...
bool PostJson(string endpoint, string json)
{
   ...
   {
      PrintFormat("EVE Momentum v2.08 POST %s failed. HTTP=%d MQL=%d response=%s", endpoint, status, GetLastError(), CharArrayToString(result));
      return false;
   }
   return true;
}
...
void RecoverPersistentState()
{
   ...
   if(CountOurPositions() > 0)
   {
      ...
      UpdateNewestCanary();
      last_event = close_pending ? "Recovered campaign with full-basket close pending" : "Recovered v2.08 price-triggered ladder campaign";
   }
   else
   {
      basket_started_at = 0;
...
{
   return StringFormat("EMB208_%I64d_%I64u_", AccountInfoInteger(ACCOUNT_LOGIN), InpMagicNumber);
}
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX+"TITLE", 10, 18, "EVE MOMENTUM BURST v2.08 - CANDLE LADDER MODE", 12, clrWhite);
