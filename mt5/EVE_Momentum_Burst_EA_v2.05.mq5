...
#property strict
#property description "M1 candle straddle and unlimited stop-order ladder. Second same-side trigger confirms; each newest SL sits beyond the prior trigger and banks the basket."
...
input string InpOrderComment                   = "EVE-MOMENTUM-V2.05";
...
input int    InpBracketArmScore                 = 0;                   // Momentum score does not gate orders
...
input int    InpReversalArmDelaySeconds         = 0;                   // Legacy input; unused in v2.05
input double InpReversalArmMinimumProfitMoney   = 0.0;                 // Legacy input; unused in v2.05

input group "Continuous pending-stop ladder"
input int    InpAddScore                        = 0;                   // Momentum score does not gate ladder orders
input double InpAddSpacingATR                   = 0.10;                // Distance between ladder stops
input int    InpAddCooldownSeconds              = 0;                   // No add cooldown
input double InpAddOnlyAfterBasketProfit        = 0.0;                 // Legacy input; unused in v2.05
input bool   InpNeverAddToLosingBasket          = false;               // Pending ladder is price-triggered, not P/L-gated
input double InpNewestLegProfitBeforeAddMoney   = 0.0;                 // Legacy input; unused in v2.05
...
int OnInit()
{
   ...
   if(trade_symbol == "")
   {
      Print("EVE Momentum v2.05: could not resolve a trading symbol.");
      return INIT_FAILED;
   }
   if(!SymbolSelect(trade_symbol, true))
   {
      Print("EVE Momentum v2.05: cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE Momentum v2.05: indicator handle creation failed. Error ", GetLastError());
      return INIT_FAILED;
   }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result)
{
   ...
   if(entry == DEAL_ENTRY_IN || entry == DEAL_ENTRY_INOUT)
   {
      ...
      if(first_leg)
      {
         ...
         campaign_phase = CAMPAIGN_OCO_CANCELLING; // v2.05: first of two same-side confirmations
...
{
   // v2.05 never adds with market orders. This helper remains only for backward compatibility.
   return false;
}
...
{
   // Legacy v2.04 hook intentionally disabled. v2.05 uses only the two-trigger
...
bool PlacePendingOrder(ENUM_ORDER_TYPE type, string role, double volume, double price, double atr_value, double protect_anchor_price, string reason)
{
   ...
   string comment = "EVE25-LAD";
   if(role == "INITIAL" || role == "ENTRY-BRACKET") comment = "EVE25-INIT";
   else if(role == "CONFIRMATION" || role == "REVERSAL") comment = "EVE25-CONF";
...
void TryAddRollingLeg()
{
   ...
   // All v2.05 additions come from broker-side pending stop orders.
}
...
void FinaliseBasketClose(string fallback_reason)
{
   ...
   campaign_cooldown_until = 0; // v2.05 restarts immediately; no strategy cooldown.
...
bool PostJson(string endpoint, string json)
{
   ...
   {
      PrintFormat("EVE Momentum v2.05 POST %s failed. HTTP=%d MQL=%d response=%s", endpoint, status, GetLastError(), CharArrayToString(result));
      return false;
   }
   return true;
}
...
void RecoverPersistentState()
{
   ...
   if(CountOurPositions() > 0)
   {
      ...
      UpdateNewestCanary();
      last_event = close_pending ? "Recovered campaign with full-basket close pending" : "Recovered v2.05 price-triggered ladder campaign";
   }
   else
   {
      basket_started_at = 0;
...
{
   return StringFormat("EMB205_%I64d_%I64u_", AccountInfoInteger(ACCOUNT_LOGIN), InpMagicNumber);
}
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX+"TITLE", 10, 18, "EVE MOMENTUM BURST v2.05 - CANDLE LADDER MODE", 12, clrWhite);
