...
#property strict
#property description "M1 candle straddle and unlimited stop-order ladder. Scores are analytics only. Second same-side trigger confirms; each newest SL banks the basket."
...
input ulong  InpMagicNumber                    = 2207202606;
input string InpOrderComment                   = "EVE-MOMENTUM-V2.06";

input group "One-time legacy isolation"
input bool   InpDeleteLegacyPendingOrders       = true;                // Deletes v2.04/v2.05 EVE pending orders before v2.06 starts
input ulong  InpLegacyMagicNumber               = 2207202603;          // Previous EVE magic number
input bool   InpBlockOnLegacyOpenPosition       = true;                // Prevents overlapping old and new EVE campaigns
...
input int    InpPostCampaignCooldownSeconds     = 0;                   // Immediate restart
input int    InpReversalArmDelaySeconds         = 0;                   // Legacy input; unused in v2.06
input double InpReversalArmMinimumProfitMoney   = 0.0;                 // Legacy input; unused in v2.06
...
input double InpAddOnlyAfterBasketProfit        = 0.0;                 // Legacy input; unused in v2.06
input bool   InpNeverAddToLosingBasket          = false;               // Pending ladder is price-triggered, not P/L-gated
input double InpNewestLegProfitBeforeAddMoney   = 0.0;                 // Legacy input; unused in v2.06
input int    InpLadderOrdersAhead                = 12;                  // Replenished continuously; campaign itself is unlimited
input double InpLadderMinimumSpacingPrice       = 0.20;
input double InpNewestSLPreviousLegLockFraction  = 0.65;                // Newest SL sits beyond the prior trigger to bank older legs
...
input double InpTakeProfitATR                   = 0.0;                 // No fixed TP; newest-leg SL banks the basket
...
int OnInit()
{
   ...
   if(trade_symbol == "")
   {
      Print("EVE Momentum v2.06: could not resolve a trading symbol.");
      return INIT_FAILED;
   }
   if(!SymbolSelect(trade_symbol, true))
   {
      Print("EVE Momentum v2.06: cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE Momentum v2.06: indicator handle creation failed. Error ", GetLastError());
      return INIT_FAILED;
   }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result)
{
   ...
   if(entry == DEAL_ENTRY_IN || entry == DEAL_ENTRY_INOUT)
   {
      ...
      if(first_leg)
      {
         ...
         campaign_phase = CAMPAIGN_OCO_CANCELLING; // v2.06: first of two same-side confirmations
   ...
   if(entry == DEAL_ENTRY_OUT || entry == DEAL_ENTRY_OUT_BY || entry == DEAL_ENTRY_INOUT)
   {
      ...
      if(deal_reason == DEAL_REASON_SL && position_id > 0 && position_id == newest_position_id && basket_started_at > 0)
      {
         newest_leg_sl_exit_detected = true;
         newest_leg_sl_exit_reason = StringFormat("NEWEST %s LEG STOP LOSS HIT - BANK ALL EARLIER POSITIONS", position_side);
         campaign_phase = CAMPAIGN_CLOSING;
         pending_sync_state = "NEWEST SL - BANKING BASKET";
         last_event = newest_leg_sl_exit_reason;
         SendEvent("newest-sl", last_event);
      }
...
bool DeleteOneForActiveLadder(string side)
{
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      ENUM_ORDER_TYPE type = (ENUM_ORDER_TYPE)OrderGetInteger(ORDER_TYPE);
      string role = SelectedPendingRole();
      if(type == wanted && role == "LADDER") continue;
      DeleteSelectedPending("second trigger confirmed; remove opposite/legacy pending order");
      return true;
   }
   return false;
}
...
double LadderSpacingPrice()
{
   ...
   double gap_for_profit_lock = broker_sl_distance / MathMax(0.20, 1.0 - lock_fraction);
   return MathMax(gap_for_profit_lock, MathMax(InpLadderMinimumSpacingPrice, CurrentOrderAtr() * InpAddSpacingATR));
}
...
void MaintainProvisionalCampaign()
{
   ...
   double spacing = LadderSpacingPrice();

   if(CountPendingBySide(side) == 0)
   {
      double base = last_add_price > 0 ? last_add_price : campaign_trigger_price;
      double desired = side == "BUY" ? base + spacing : base - spacing;
      pending_sync_state = "PLACING SECOND SAME-SIDE STOP";
      PlacePendingOrder(StopTypeForSide(side), "CONFIRMATION", lot, desired, atr_value, base, "second same-side trigger confirms momentum; SL locks the first leg in gross profit");
      return;
   }
   ...
   pending_sync_state = StringFormat("PROVISIONAL %s 1/2 - BOTH STOPS LIVE", side);
   last_event = StringFormat("%s is provisional: waiting for second %s trigger; opposite %s stop remains active", side, side, opposite);
}
...
void MaintainActiveLadder()
{
   ...
   int wanted_ahead = MathMax(1, InpLadderOrdersAhead);
   int existing = CountPendingBySideAndRole(side, "LADDER");
   if(existing >= wanted_ahead)
   {
      pending_sync_state = StringFormat("%s LADDER READY - %d STOPS AHEAD", side, existing);
      return;
   }

   double base = last_add_price;
   double extreme = ExtremePendingPrice(side, "LADDER");
   if(extreme > 0)
   {
      if(side == "BUY") base = MathMax(base, extreme);
      else if(base == 0) base = extreme;
      else base = MathMin(base, extreme);
   }
   if(base <= 0) base = AverageBasketEntry();

   double desired = side == "BUY" ? base + LadderSpacingPrice() : base - LadderSpacingPrice();
   pending_sync_state = StringFormat("BUILDING %s LADDER %d/%d", side, existing, wanted_ahead);
   PlacePendingOrder(StopTypeForSide(side), "LADDER", CalculateLegLot(), desired, CurrentOrderAtr(), base, "continuous stop-order ladder; newest SL sits beyond the prior trigger");
}
...
void ApplyOperationalBlocks(MomentumSnapshot &scan)
{
   ...
   if(CountOurPositions() > 0)
   {
      scan.block_reason = campaign_phase == CAMPAIGN_OCO_CANCELLING
         ? "PRICE ENGINE RUNNING - FIRST TRIGGER PROVISIONAL; SECOND SAME-SIDE TRIGGER CONFIRMS"
         : "PRICE ENGINE RUNNING - CONTINUOUS SAME-SIDE STOP LADDER";
      return;
   }

   scan.block_reason = "24/5 PRICE ENGINE RUNNING - FRESH BUY STOP AND SELL STOP EACH CANDLE";
}


bool OpenMarketLeg(ENUM_POSITION_TYPE direction, double volume, double atr_value)
{
   // v2.06 never adds with market orders. This helper remains only for backward compatibility.
   return false;
}
...
{
   return StringFind(comment, "EVE-MB2-") >= 0 || StringFind(comment, "EVE25-") >= 0 || StringFind(comment, "EVE-MOMENTUM-V2.04") >= 0 || StringFind(comment, "EVE-MOMENTUM-V2.05") >= 0;
}
...
bool HandleLegacyPendingCleanup()
{
   ...
   last_event = "v2.06 is deleting v2.04/v2.05 pending orders before starting its own bracket";
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      delete_wait_ticket = ticket;
      delete_wait_started_ms = GetTickCount64();
      SendOrderActivity("LEGACY-CANCEL-REQUESTED", "LEGACY", type, ticket, volume, price, "v2.06 version isolation cleanup");
      return true;
   }
   return true;
}
...
void ManageBasket()
{
   ...
   if(InpBlockOnLegacyOpenPosition && CountLegacyEvePositions() > 0)
   {
      ...
      pending_sync_state = "CLOSE OLD V2.04/V2.05 POSITION BEFORE V2.06";
      last_event = "v2.06 is isolated and will not overlap an open position from the previous EVE magic number";
      CancelPendingOrders();
      return;
   }
   ...
   {
      string reason = newest_leg_sl_exit_reason == "" ? "NEWEST LADDER LEG STOP LOSS HIT - BANK ALL" : newest_leg_sl_exit_reason;
   ...
   bank_candidate = true;
   bank_candidate_reason = "The newest triggered position SL is the full-basket exit switch";
   ...
   campaign_phase = CAMPAIGN_ACTIVE;
   engine_state = "CONTINUOUS LADDER";
...
void MaintainMovingStraddle()
{
   ...
   if(straddle_candle_time != 0 && current_bar != straddle_candle_time)
   {
      if(CountOurPendingOrders() > 0)
      {
         pending_sync_state = "NEW CANDLE - REPLACING STRADDLE";
         CancelPendingOrders();
         return;
      }
      ResetStraddleAnchor();
   ...
   if(straddle_candle_time == 0 && !CaptureCurrentCandleAnchor()) return;
   ...
   if(CountPendingBySideAndRole("BUY", "INITIAL") == 0)
   {
      campaign_phase = CAMPAIGN_BUILDING_STRADDLE;
      pending_sync_state = "PLACING CANDLE BUY STOP";
      PlacePendingOrder(ORDER_TYPE_BUY_STOP, "INITIAL", lot, desired_buy, atr_value, 0.0, "fresh execution-timeframe candle BUY STOP");
      return;
   }

   if(CountPendingBySideAndRole("SELL", "INITIAL") == 0)
   {
      campaign_phase = CAMPAIGN_BUILDING_STRADDLE;
      pending_sync_state = "PLACING CANDLE SELL STOP";
      PlacePendingOrder(ORDER_TYPE_SELL_STOP, "INITIAL", lot, desired_sell, atr_value, 0.0, "fresh execution-timeframe candle SELL STOP");
      return;
   }

   last_bracket_buy_price = ExtremePendingPrice("BUY", "INITIAL");
   last_bracket_sell_price = ExtremePendingPrice("SELL", "INITIAL");
...
{
   // Legacy v2.04 hook intentionally disabled. v2.06 uses only the two-trigger
...
bool PlacePendingOrder(ENUM_ORDER_TYPE type, string role, double volume, double price, double atr_value, double protect_anchor_price, string reason)
{
   ...
   if(!BuildSafePendingLevels(type, role, price, atr_value, protect_anchor_price, safe_price, sl, tp)) return false;

   string comment = "EVE26-LAD";
   if(role == "INITIAL" || role == "ENTRY-BRACKET") comment = "EVE26-INIT";
   else if(role == "CONFIRMATION" || role == "REVERSAL") comment = "EVE26-CONF";
...
void TryAddRollingLeg()
{
   ...
   // All v2.06 additions come from broker-side pending stop orders.
}
...
void TrailAllPositions()
{
   ...
   // submitted with its pending order; the newest-leg SL controls basket banking.
}
...
void ContinuePendingClose()
{
   ...
   // Bank open profit first. Any pending order that triggers during closure is also absorbed and closed.
   if(CountOurPositions() > 0)
   {
   ...
   {
      CancelPendingOrders();
      last_event = StringFormat("BANKING PENDING - positions closed; clearing %d pending ladder order(s)", CountOurPendingOrders());
      SavePersistentState();
      return;
   }
}
...
void FinaliseBasketClose(string fallback_reason)
{
   ...
   campaign_cooldown_until = 0; // v2.06 restarts immediately; no strategy cooldown.
...
bool BuildSafePendingLevels(ENUM_ORDER_TYPE type, string role, double requested_price, double atr_value, double protect_anchor_price, double &entry, double &sl, double &tp)
{
   MqlTick tick;
   ...
   // Confirmation and ladder stops are widened when necessary so their own SL can sit
   // beyond the previous trigger. This makes the newest SL a genuine profit-banking line
   // for every older same-direction position under normal fills.
   if(profit_lock_role)
   {
      double lock_fraction = MathMax(0.10, MathMin(0.80, InpNewestSLPreviousLegLockFraction));
      double minimum_lock = MathMax(2.0 * tick_size, SymbolInfoDouble(trade_symbol, SYMBOL_POINT) * 2.0);
      double required_gap = MathMax(stops_distance + minimum_lock, (stops_distance + tick_size) / MathMax(0.20, 1.0 - lock_fraction));
      if(type == ORDER_TYPE_BUY_STOP)
         entry = RoundPendingPrice(type, MathMax(entry, protect_anchor_price + required_gap));
      else
         entry = RoundPendingPrice(type, MathMin(entry, protect_anchor_price - required_gap));
   }
   ...
   if(profit_lock_role)
   {
      ...
      double candidate = type == ORDER_TYPE_BUY_STOP
         ...
         : protect_anchor_price - gap * lock_fraction;
      if(type == ORDER_TYPE_BUY_STOP)
         sl = NormalizePrice(MathMin(candidate, entry - stops_distance));
      else
         sl = NormalizePrice(MathMax(candidate, entry + stops_distance));
   }
   else
   {
   ...
   tp = 0.0;
   bool correct_side = type == ORDER_TYPE_BUY_STOP ? sl < entry : sl > entry;
   bool prior_leg_locked = !profit_lock_role || (type == ORDER_TYPE_BUY_STOP ? sl > protect_anchor_price : sl < protect_anchor_price);
   return entry > 0 && sl > 0 && correct_side && prior_leg_locked;
}
...
bool ModifyPendingOrder(ulong ticket, ENUM_ORDER_TYPE type, string role, double desired_price, double atr_value, string reason)
{
   ...
   double safe_price = 0.0, sl = 0.0, tp = 0.0;
   if(!BuildSafePendingLevels(type, role, desired_price, atr_value, 0.0, safe_price, sl, tp)) return false;
   MarkTradeRequest();
...
bool PostJson(string endpoint, string json)
{
   ...
   {
      PrintFormat("EVE Momentum v2.06 POST %s failed. HTTP=%d MQL=%d response=%s", endpoint, status, GetLastError(), CharArrayToString(result));
      return false;
   }
   return true;
}
...
void RecoverPersistentState()
{
   ...
   if(CountOurPositions() > 0)
   {
      ...
      UpdateNewestCanary();
      last_event = close_pending ? "Recovered campaign with full-basket close pending" : "Recovered v2.06 price-triggered ladder campaign";
   }
   else
   {
      basket_started_at = 0;
...
{
   return StringFormat("EMB206_%I64d_%I64u_", AccountInfoInteger(ACCOUNT_LOGIN), InpMagicNumber);
}
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX+"TITLE", 10, 18, "EVE MOMENTUM BURST v2.06 - CANDLE LADDER MODE", 12, clrWhite);
