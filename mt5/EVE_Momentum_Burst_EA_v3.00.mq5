...
input ulong  InpMagicNumber                    = 2207202630;
input string InpOrderComment                   = "EVE-MOMENTUM-V3.00";

input group "One-time legacy isolation"
input bool   InpDeleteLegacyPendingOrders       = true;                // Deletes recognised older EVE pending orders before v3.00 starts
input ulong  InpLegacyMagicNumber               = 2207202611;          // Previous v2.11 magic
input ulong  InpOlderLegacyMagicNumber          = 2207202610;          // Older v2.10 magic
input ulong  InpOldestLegacyMagicNumber         = 2207202609;          // Older v2.09 magic
input bool   InpBlockOnLegacyOpenPosition       = true;                // Prevents overlapping old EVE and v3.00 campaigns
...
input bool   InpDirectionalScoutOnly            = true;                // v3.00 places only the stop matching the detected burst direction
...
input bool   InpOneCampaignAttemptPerCandle      = false;               // Legacy input; unused. v3.00 is tick-burst driven, not candle driven
input bool   InpPausePendingEntriesOnWideSpread  = true;                // Execution safety only; open positions keep their broker-side SL
input int    InpReversalArmDelaySeconds         = 0;                   // Legacy input; unused in v3.00
input double InpReversalArmMinimumProfitMoney   = 0.0;                 // Legacy input; unused in v3.00
...
input double InpAddOnlyAfterBasketProfit        = 0.0;                 // Legacy input; unused in v3.00
input bool   InpNeverAddToLosingBasket          = false;               // Pending ladder is price-triggered, not P/L-gated
input double InpNewestLegProfitBeforeAddMoney   = 0.0;                 // Legacy input; unused in v3.00
...
input double InpBreakEvenTriggerATR             = 0.10;                 // Legacy input; unused in v3.00
input double InpTrailingActivationATR           = 0.16;                 // Legacy input; unused in v3.00
input double InpNewestTrailATR                  = 0.065;                // Legacy input; unused in v3.00
input double InpMiddleTrailATR                  = 0.10;                 // Legacy input; unused in v3.00
input double InpOldestTrailATR                  = 0.16;                 // Legacy input; unused in v3.00
...
// v3.00 tick-burst, provisional-lock, ladder and shared-stop state.
...
int OnInit()
{
   ...
   if(trade_symbol == "")
   {
      Print("EVE Momentum v3.00: could not resolve a trading symbol.");
      return INIT_FAILED;
   }
   if(!SymbolSelect(trade_symbol, true))
   {
      Print("EVE Momentum v3.00: cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE Momentum v3.00: indicator handle creation failed. Error ", GetLastError());
      return INIT_FAILED;
   }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result)
{
   ...
   if(entry == DEAL_ENTRY_IN || entry == DEAL_ENTRY_INOUT)
   {
      ...
      if(first_leg)
      {
         ...
         campaign_phase = CAMPAIGN_OCO_CANCELLING; // v3.00: protected directional scout
...
bool PlacePendingOrderExact(ENUM_ORDER_TYPE type, string role, double volume, double entry, double sl, string reason)
{
   ...
   string comment = "EVE3-LAD";
   if(role == "INITIAL" || role == "ENTRY-BRACKET") comment = "EVE3-INIT";
   else if(role == "CONFIRMATION" || role == "REVERSAL") comment = "EVE3-CONF";
...
void MaintainProvisionalCampaign()
{
   ...
   // v3.00 never flips into the opposite direction inside the same campaign.
...
{
   // v3.00 never adds with market orders. This helper remains only for backward compatibility.
   return false;
}
...
{
   return magic == InpLegacyMagicNumber || magic == InpOlderLegacyMagicNumber || magic == InpOldestLegacyMagicNumber;
}

bool IsLegacyEveComment(string comment)
{
   return StringFind(comment, "EVE31-") >= 0 || StringFind(comment, "EVE-MOMENTUM-V2.11") >= 0 ||
...
bool HandleLegacyPendingCleanup()
{
   ...
   last_event = "v3.00 is deleting recognised older EVE pending orders before starting";
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      delete_wait_ticket = ticket;
      delete_wait_started_ms = GetTickCount64();
      SendOrderActivity("LEGACY-CANCEL-REQUESTED", "LEGACY", type, ticket, volume, price, "v3.00 version isolation cleanup");
      return true;
   }
   return true;
}
...
void ManageBasket()
{
   ...
   if(InpBlockOnLegacyOpenPosition && CountLegacyEvePositions() > 0)
   {
      ...
      pending_sync_state = "CLOSE OLD EVE POSITION BEFORE V3.00";
      last_event = "v3.00 is isolated and will not overlap any recognised older EVE position";
      CancelPendingOrders();
      return;
   }
   ...
   // Any mixed direction is an execution safety event. v3.00 never flips or hedges inside a campaign.
...
void MaintainMovingStraddle()
{
   ...
   if(!burst_bracket_armed)
   {
      ...
      // one-second direction. v3.00 then arms only that directional scout stop by default.
   ...
   // Optional research fallback: two-sided bracket. Directional scout mode is the v3.00 default.
...
{
   // Legacy reversal hook intentionally disabled. v3.00 never flips direction inside an active campaign.
}
...
void TryAddRollingLeg()
{
   ...
   // All v3.00 additions come from one broker-side pending stop order ahead.
}
...
{
   string json = StringFormat("{\"id\":\"%I64u-%s\",\"basketId\":\"%I64d\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"3.00\",\"strategy\":\"SCOUT_PROVEN_LADDER\",\"magic\":\"%I64u\",\"action\":\"%s\",\"side\":\"%s\",\"ticket\":\"%I64u\",\"positionId\":\"%I64u\",\"volume\":%.2f,\"price\":%.5f,\"netProfit\":%.2f,\"reason\":\"%s\",\"dealTime\":%I64d}", deal_ticket, JsonEscape(action), basket_id, AccountInfoInteger(ACCOUNT_LOGIN), JsonEscape(trade_symbol), InpMagicNumber, JsonEscape(action), JsonEscape(side), deal_ticket, position_id, volume, price, net, JsonEscape(reason), (long)TimeCurrent() * 1000);
...
void SendOrderActivity(string action, string role, ENUM_ORDER_TYPE type, ulong ticket, double volume, double price, string reason)
{
   ...
   string json = StringFormat("{\"id\":\"%I64d-%I64u-%s\",\"basketId\":\"%I64d\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"3.00\",\"strategy\":\"SCOUT_PROVEN_LADDER\",\"magic\":\"%I64u\",\"action\":\"%s\",\"role\":\"%s\",\"orderType\":\"%s\",\"ticket\":\"%I64u\",\"volume\":%.2f,\"price\":%.5f,\"reason\":\"%s\",\"at\":%I64d}", (long)TimeCurrent(), ticket, JsonEscape(action), basket_id, AccountInfoInteger(ACCOUNT_LOGIN), JsonEscape(trade_symbol), InpMagicNumber, JsonEscape(action), JsonEscape(role), order_type, ticket, volume, price, JsonEscape(reason), (long)TimeCurrent() * 1000);
   QueueJson("/api/ea/order", json);
}

void SendBankDecision(string reason, double profit, int same_score, int opposite_score)
{
   string json = StringFormat("{\"id\":\"%I64d-%I64d\",\"basketId\":\"%I64d\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"3.00\",\"strategy\":\"SCOUT_PROVEN_LADDER\",\"magic\":\"%I64u\",\"side\":\"%s\",\"basketProfit\":%.2f,\"peakBasketProfit\":%.2f,\"newestTicket\":\"%I64u\",\"newestProfit\":%.2f,\"newestPeak\":%.2f,\"sameScore\":%d,\"oppositeScore\":%d,\"momentumState\":\"%s\",\"reason\":\"%s\",\"at\":%I64d}", (long)TimeCurrent(), basket_id, basket_id, AccountInfoInteger(ACCOUNT_LOGIN), JsonEscape(trade_symbol), InpMagicNumber, JsonEscape(BasketSideText()), profit, peak_basket_profit, newest_ticket, newest_leg_current_profit, newest_leg_peak_profit, same_score, opposite_score, have_scan ? JsonEscape(last_scan.momentum_state) : "UNKNOWN", JsonEscape(reason), (long)TimeCurrent() * 1000);
...
void ContinuePendingClose()
{
   ...
   // v3.00 quarantines the campaign first: pending entries are cancelled before
...
void FinaliseBasketClose(string fallback_reason)
{
   ...
   string json = StringFormat(
      "{\"id\":\"%I64d-%I64u\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"3.00\",\"strategy\":\"SCOUT_PROVEN_LADDER\",\"magic\":\"%I64u\",\"side\":\"%s\",\"startSide\":\"%s\",\"finalSide\":\"%s\",\"buyLegs\":%d,\"sellLegs\":%d,\"reversalCount\":%d,\"volume\":%.2f,\"lotPerLeg\":%.2f,\"maxTotalLotsUsed\":%.2f,\"positionsOpened\":%d,\"maxConcurrentPositions\":%d,\"entryTime\":%I64d,\"exitTime\":%I64d,\"entryPrice\":%.5f,\"exitPrice\":%.5f,\"entryScore\":%d,\"oppositeScore\":%d,\"entryRegime\":\"%s\",\"entryState\":\"%s\",\"entryReason\":\"%s\",\"exitReason\":\"%s\",\"netProfit\":%.2f,\"peakBasketProfit\":%.2f,\"profitGiveback\":%.2f,\"mfe\":%.2f,\"mae\":%.2f,\"durationSeconds\":%d,\"closeAttempts\":%d,\"closeTriggerProfit\":%.2f,\"newestTicket\":\"%I64u\",\"newestProfitAtBank\":%.2f,\"newestPeak\":%.2f,\"reversalTriggered\":%s,\"status\":\"CLOSED\"}",
...
bool PostJson(string endpoint, string json)
{
   ...
   {
      PrintFormat("EVE Momentum v3.00 POST %s failed. HTTP=%d MQL=%d response=%s", endpoint, status, GetLastError(), CharArrayToString(result));
      return false;
   }
   return true;
}
...
void RecoverPersistentState()
{
   ...
   if(CountOurPositions() > 0)
   {
      ...
      UpdateNewestCanary();
      last_event = close_pending ? "Recovered campaign with full-basket close pending" : "Recovered v3.00 price-triggered ladder campaign";
   }
   else
   {
      basket_started_at = 0;
...
{
   return StringFormat("EMB300_%I64d_%I64u_", AccountInfoInteger(ACCOUNT_LOGIN), InpMagicNumber);
}
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX+"TITLE", 10, 18, "EVE MOMENTUM BURST v3.00 - SCOUT-PROVEN LADDER", 12, clrWhite);
