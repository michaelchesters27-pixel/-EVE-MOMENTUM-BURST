...
int OnInit()
{
   ...
   if(trade_symbol == "")
   {
      Print("EVE Momentum v2.02: could not resolve a trading symbol.");
      return INIT_FAILED;
   }
   if(!SymbolSelect(trade_symbol, true))
   {
      Print("EVE Momentum v2.02: cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE Momentum v2.02: indicator handle creation failed. Error ", GetLastError());
      return INIT_FAILED;
   }
...
void MaintainMovingStraddle()
{
   ...
   if(baskets_today >= InpMaximumBasketsPerDay) { pending_sync_state = "CLEARING"; CancelPendingOrders(); last_event = "Daily basket limit reached"; return; }
   if(consecutive_losses >= InpMaximumConsecutiveLosses) { pending_sync_state = "CLEARING"; CancelPendingOrders(); last_event = "Consecutive loss limit reached"; return; }
...
bool PostJson(string endpoint, string json)
{
   ...
   {
      PrintFormat("EVE Momentum v2.02 POST %s failed. HTTP=%d MQL=%d response=%s", endpoint, status, GetLastError(), CharArrayToString(result));
      return false;
   }
   return true;
}
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX+"TITLE", 10, 18, "EVE MOMENTUM BURST v2.02 - EXECUTION SAFE", 12, clrWhite);
