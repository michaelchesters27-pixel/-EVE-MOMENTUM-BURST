...
#property strict
#property description "Tick-by-tick acceleration arming with M1 volatility context, position 1 profit lock, one ladder order ahead and a shared broker-side basket SL."
...
input ulong  InpMagicNumber                    = 2207202611;
input string InpOrderComment                   = "EVE-MOMENTUM-V2.11";

input group "One-time legacy isolation"
input bool   InpDeleteLegacyPendingOrders       = true;                // Deletes recognised v2.09/v2.10 EVE pending orders before v2.11 starts
input ulong  InpLegacyMagicNumber               = 2207202610;          // Previous v2.10 magic
input ulong  InpOlderLegacyMagicNumber          = 2207202609;          // Older v2.09 magic
input bool   InpBlockOnLegacyOpenPosition       = true;                // Prevents overlapping old EVE and v2.11 campaigns
...
input bool   InpOneCampaignAttemptPerCandle      = false;               // Legacy input; unused. v2.11 is tick-burst driven, not candle driven
input bool   InpPausePendingEntriesOnWideSpread  = true;                // Execution safety only; open positions keep their broker-side SL
input int    InpReversalArmDelaySeconds         = 0;                   // Legacy input; unused in v2.11
input double InpReversalArmMinimumProfitMoney   = 0.0;                 // Legacy input; unused in v2.11
...
input double InpAddOnlyAfterBasketProfit        = 0.0;                 // Legacy input; unused in v2.11
input bool   InpNeverAddToLosingBasket          = false;               // Pending ladder is price-triggered, not P/L-gated
input double InpNewestLegProfitBeforeAddMoney   = 0.0;                 // Legacy input; unused in v2.11
...
input double InpBreakEvenTriggerATR             = 0.10;                 // Legacy input; unused in v2.11
input double InpTrailingActivationATR           = 0.16;                 // Legacy input; unused in v2.11
input double InpNewestTrailATR                  = 0.065;                // Legacy input; unused in v2.11
input double InpMiddleTrailATR                  = 0.10;                 // Legacy input; unused in v2.11
input double InpOldestTrailATR                  = 0.16;                 // Legacy input; unused in v2.11
...
// v2.11 tick-burst, provisional-lock, ladder and shared-stop state.
...
int OnInit()
{
   ...
   if(trade_symbol == "")
   {
      Print("EVE Momentum v2.11: could not resolve a trading symbol.");
      return INIT_FAILED;
   }
   if(!SymbolSelect(trade_symbol, true))
   {
      Print("EVE Momentum v2.11: cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE Momentum v2.11: indicator handle creation failed. Error ", GetLastError());
      return INIT_FAILED;
   }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result)
{
   ...
   if(entry == DEAL_ENTRY_IN || entry == DEAL_ENTRY_INOUT)
   {
      ...
      if(first_leg)
      {
         ...
         campaign_phase = CAMPAIGN_OCO_CANCELLING; // v2.11: first of two same-side confirmations
         basket_entry_atr = CurrentOrderAtr();
         ...
         basket_entry_reason = "FIRST TICK-BURST STOP TRIGGER - DIRECTION PROVISIONAL";
         baskets_today++;
...
bool DeleteOneForFlatStraddle()
{
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      if(keep) continue;
      DeleteSelectedPending("tick-burst bracket duplicate cleanup");
      return true;
   }
   return false;
}
...
bool PlacePendingOrderExact(ENUM_ORDER_TYPE type, string role, double volume, double entry, double sl, string reason)
{
   ...
   string comment = "EVE31-LAD";
   if(role == "INITIAL" || role == "ENTRY-BRACKET") comment = "EVE31-INIT";
   else if(role == "CONFIRMATION" || role == "REVERSAL") comment = "EVE31-CONF";
...
void ApplyOperationalBlocks(MomentumSnapshot &scan)
{
   ...
   scan.block_reason = burst_bracket_armed ? "LIVE BURST DETECTED - TWO-SIDED BREAKOUT BRACKET ARMED" : "WATCHING LIVE TICKS FOR ACCELERATION";
}


bool OpenMarketLeg(ENUM_POSITION_TYPE direction, double volume, double atr_value)
{
   // v2.11 never adds with market orders. This helper remains only for backward compatibility.
   return false;
}
...
{
   return magic == InpLegacyMagicNumber || magic == InpOlderLegacyMagicNumber;
}

bool IsLegacyEveComment(string comment)
{
   return StringFind(comment, "EVE30-") >= 0 || StringFind(comment, "EVE-MOMENTUM-V2.10") >= 0 ||
...
{
   if(InpLegacyMagicNumber == InpMagicNumber && InpOlderLegacyMagicNumber == InpMagicNumber) return 0;
   int count = 0;
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ulong ticket = PositionGetTicket(i);
...
bool HandleLegacyPendingCleanup()
{
   ...
   last_event = "v2.11 is deleting recognised v2.09/v2.10 pending orders before starting";
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      delete_wait_ticket = ticket;
      delete_wait_started_ms = GetTickCount64();
      SendOrderActivity("LEGACY-CANCEL-REQUESTED", "LEGACY", type, ticket, volume, price, "v2.11 version isolation cleanup");
      return true;
   }
   return true;
}
...
void ManageBasket()
{
   ...
   if(InpBlockOnLegacyOpenPosition && CountLegacyEvePositions() > 0)
   {
      ...
      pending_sync_state = "CLOSE OLD EVE POSITION BEFORE V2.11";
      last_event = "v2.11 is isolated and will not overlap an open v2.09/v2.10 position";
      CancelPendingOrders();
      return;
   }
   ...
   if(positions == 0)
   {
      ...
      engine_state = "TICK-BURST ENGINE";
...
void MaintainMovingStraddle()
{
   ...
   if(!burst_bracket_armed)
   {
      ...
      // one-second direction. The bracket itself remains two-sided.
   ...
   campaign_phase = CAMPAIGN_BUILDING_STRADDLE;
   engine_state = "LIVE TICK-BURST BRACKET";

   if(!FindOurPendingOrder(ORDER_TYPE_BUY_STOP, ticket, price, sl, tp, setup))
   {
      pending_sync_state = "PLACING BURST BUY STOP";
      if(!PlacePendingOrder(ORDER_TYPE_BUY_STOP, "INITIAL", lot, desired_buy, atr_value, 0.0, "tick-burst BUY STOP"))
         return;
   }

   // With a zero strategy request cooldown, submit the matching SELL STOP in the
   // same processing pass whenever the broker accepts the BUY STOP. This sharply
   // reduces the temporary one-sided window.
   if(!FindOurPendingOrder(ORDER_TYPE_SELL_STOP, ticket, price, sl, tp, setup))
   {
      pending_sync_state = "PLACING BURST SELL STOP";
      if(!PlacePendingOrder(ORDER_TYPE_SELL_STOP, "INITIAL", lot, desired_sell, atr_value, 0.0, "tick-burst SELL STOP"))
         return;
   }
   ...
   pending_sync_state = "TICK-BURST BRACKET READY";
   last_event = StringFormat("%s burst bracket ready: BUY STOP %.2f / SELL STOP %.2f",
...
{
   // Legacy v2.04 hook intentionally disabled. v2.11 uses only the two-trigger
...
void TryAddRollingLeg()
{
   ...
   // All v2.11 additions come from one broker-side pending stop order ahead.
}
...
void ContinuePendingClose()
{
   ...
   // v2.11 quarantines the campaign first: pending entries are cancelled before
...
bool PostJson(string endpoint, string json)
{
   ...
   {
      PrintFormat("EVE Momentum v2.11 POST %s failed. HTTP=%d MQL=%d response=%s", endpoint, status, GetLastError(), CharArrayToString(result));
      return false;
   }
   return true;
}
...
void RecoverPersistentState()
{
   ...
   if(CountOurPositions() > 0)
   {
      ...
      UpdateNewestCanary();
      last_event = close_pending ? "Recovered campaign with full-basket close pending" : "Recovered v2.11 price-triggered ladder campaign";
   }
   else
   {
      basket_started_at = 0;
...
{
   return StringFormat("EMB211_%I64d_%I64u_", AccountInfoInteger(ACCOUNT_LOGIN), InpMagicNumber);
}
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX+"TITLE", 10, 18, "EVE MOMENTUM BURST v2.11 - LIVE TICK BURST ENGINE", 12, clrWhite);
