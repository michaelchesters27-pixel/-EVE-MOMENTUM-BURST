...
int OnInit()
{
   ...
   if(trade_symbol == "")
   {
      Print("EVE Momentum v2.03: could not resolve a trading symbol.");
      return INIT_FAILED;
   }
   if(!SymbolSelect(trade_symbol, true))
   {
      Print("EVE Momentum v2.03: cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE Momentum v2.03: indicator handle creation failed. Error ", GetLastError());
      return INIT_FAILED;
   }
...
{
   // Clear the delete lock only after the terminal confirms that the ticket left the active order pool.
...
void ManageBasket()
{
   ...
   if(positions == 0)
   {
      ...
      if(basket_started_at > 0)
      {
         ...
         {
            pending_sync_state = "CLEARING FLAT BASKET";
            CancelPendingOrders();
            last_event = StringFormat("Basket is flat; clearing %d pending order(s) before final report", CountOurPendingOrders());
            return;
         }
...
void MaintainMovingStraddle()
{
   ...
   if(best_score < InpBracketArmScore || last_scan.regime == "CHAOTIC")
   {
      pending_sync_state = "CLEARING";
      CancelPendingOrders();
      last_event = "Waiting for enough live movement to arm the straddle";
      return;
   }

   MqlTick tick;
   if(!SymbolInfoTick(trade_symbol, tick)) return;
   double distance = MathMax(last_scan.atr * InpBracketDistanceATR, PendingEntryDistancePrice());
   ...
   pending_sync_state = "SYNCING STRADDLE";
   bool ready = SyncPendingOrderSet(true, true, desired_buy, desired_sell, lot, last_scan.atr, "ENTRY-BRACKET", "moving breakout bracket");
   if(ready)
   {
      ...
      pending_sync_state = "STRADDLE READY";
      last_event = StringFormat("Moving straddle ready: BUY STOP %.2f / SELL STOP %.2f", last_bracket_buy_price, last_bracket_sell_price);
   }
}

void MaintainReverseStop()
{
   if(!remote_autonomous || local_paused || emergency_stopped || close_pending || CountOurPositions() <= 0)
   {
      pending_sync_state = "CLEARING";
      CancelPendingOrders();
      return;
   ...
   double protected_stop = BasketProtectedStop();
   double desired = 0.0;
   ENUM_ORDER_TYPE type;
   if(side == "BUY")
   {
      type = ORDER_TYPE_SELL_STOP;
      desired = protected_stop > 0 ? protected_stop - atr_value * InpReverseBufferATR : AverageBasketEntry() - atr_value * InpReverseStopATR;
   ...
   pending_sync_state = "SYNCING REVERSAL";
   bool ready = type == ORDER_TYPE_BUY_STOP
      ? SyncPendingOrderSet(true, false, desired, 0.0, lot, atr_value, "REVERSAL", "opposite stop aligned to ladder invalidation")
      : SyncPendingOrderSet(false, true, 0.0, desired, lot, atr_value, "REVERSAL", "opposite stop aligned to ladder invalidation");
   if(ready)
   {
      pending_sync_state = "REVERSAL READY";
      last_event = StringFormat("%s reversal stop ready at %.2f", type == ORDER_TYPE_BUY_STOP ? "BUY" : "SELL", desired);
   }
}
...
bool ResolveMixedDirectionIfNeeded()
{
   ...
   {
      engine_state = "REVERSAL SYNC";
      last_event = "Stop-and-reverse transition is waiting for the previous broker request";
      return true;
   }
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      {
         RegisterPendingFailure(code);
         last_event = StringFormat("Reversal synchronisation failed for %I64u: %u %s", ticket, code, trade.ResultRetcodeDescription());
      }
      else
      {
         basket_side = new_side;
         last_event = "Stop-and-reverse triggered; one old-direction leg is closing and " + new_side + " is retained";
...
void SendHeartbeat()
{
   ...
   string json = StringFormat(
      ...
      JsonEscape(engine_state), JsonEscape(pending_sync_state),
...
bool PostJson(string endpoint, string json)
{
   ...
   {
      PrintFormat("EVE Momentum v2.03 POST %s failed. HTTP=%d MQL=%d response=%s", endpoint, status, GetLastError(), CharArrayToString(result));
      return false;
   }
   return true;
}
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX+"TITLE", 10, 18, "EVE MOMENTUM BURST v2.03 - DEMO TEST MODE", 12, clrWhite);
