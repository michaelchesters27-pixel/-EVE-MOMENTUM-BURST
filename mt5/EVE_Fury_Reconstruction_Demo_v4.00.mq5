...
#property strict
#property description "Fury-style XAUUSD demo reconstruction: tick-burst entry, rapid same-direction stop ladder, individual broker SL/TP, break-even and adaptive trailing."
...
input ulong  InpMagicNumber                    = 2407202640;
input string InpOrderComment                   = "EVE-FURY-DEMO-V4.00";

input group "One-time legacy isolation"
input bool   InpDeleteLegacyPendingOrders       = true;                // Deletes recognised older EVE pending orders before v4.00 starts
input ulong  InpLegacyMagicNumber               = 2207202631;          // Previous v3.01 magic
input ulong  InpOlderLegacyMagicNumber          = 2207202630;          // Older v3.00 magic
input ulong  InpOldestLegacyMagicNumber         = 2207202611;          // Older v2.11 magic
input ulong  InpAncientLegacyMagicNumber        = 2207202610;          // Older v2.10 magic
input bool   InpBlockOnLegacyOpenPosition       = true;                // Prevents overlapping old EVE and v4.00 campaigns
...
input int    InpMaximumPositions                = 10;                  // 0 = unlimited; retained only for dashboard compatibility
...
input group "Deterministic execution supervisor"
input bool   InpStrictStateMachine                = true;                // Rebuilds state from live MT5 positions/orders every pass
input bool   InpUseServerPendingExpiration        = true;                // Uses broker-side expiry when the symbol supports it
input int    InpPendingTransitionTimeoutSeconds   = 4;                   // Order request state may not remain transitional indefinitely
input int    InpWrongSideRetryMilliseconds        = 500;                 // Broker-recovery pacing only; not a strategy cooldown
input int    InpMaximumFirstFillSlippagePoints    = 40;                  // Reject first fills materially beyond the planned stop price
input double InpMaximumFirstFillSlippageATR       = 0.06;                // ATR-based first-fill slippage ceiling
input bool   InpAutoRecoverWhenFlat               = true;                // Clears a recovery latch only after MT5 is truly flat and order-free

input group "Tick-burst breakout arming"
input bool   InpUseMovingStraddle               = true;                // Arms only after a genuine live tick burst
input bool   InpDirectionalScoutOnly            = true;                // Legacy display input; v4.00 always uses one directional scout order
input double InpBracketDistanceATR              = 0.20;
input double InpBracketRefreshATR               = 0.10;
input int    InpBracketMaximumAgeSeconds        = 0;                   // Legacy input; burst bracket lifetime is controlled below
...
input int    InpPostCampaignCooldownSeconds     = 0;                   // Legacy input; unused. Rearming requires a quiet reset, not a timed cooldown
input bool   InpOneCampaignAttemptPerCandle      = false;               // Legacy input; unused. v4.00 is tick-burst driven, not candle driven
input bool   InpPausePendingEntriesOnWideSpread  = true;                // Execution safety only; open positions keep their broker-side SL
input int    InpReversalArmDelaySeconds         = 0;                   // Legacy input; unused in v4.00
input double InpReversalArmMinimumProfitMoney   = 0.0;                 // Legacy input; unused in v4.00

input double InpBurstArmVelocity1ATR             = 0.025;               // 1-second price speed needed to arm
input double InpBurstArmVelocity3ATR             = 0.012;               // 3-second price speed needed to arm
input double InpBurstArmAcceleration             = 1.12;                // 1-second speed / 3-second speed ratio
input double InpBurstArmTickExpansion             = 1.18;                // Recent tick arrival rate / 30-second baseline
input bool   InpUseM5SoftBias                    = true;                // Counter-trend bursts must be stronger, but are not forbidden
input double InpM5CounterTrendBurstMultiplier    = 1.25;                // Extra strength required against M5 direction
input int    InpBurstBracketLifetimeSeconds       = 20;                  // If neither side triggers, disarm and wait for a fresh burst
input double InpBurstResetQuietVelocityATR        = 0.010;               // Market must quieten before another burst can arm
input double InpBurstAnchorLookbackSeconds        = 3.0;                 // Use recent micro high/low around the burst
...
input double InpAddOnlyAfterBasketProfit        = 0.0;                 // Legacy input; unused in v4.00
input bool   InpNeverAddToLosingBasket          = false;               // Pending ladder is price-triggered, not P/L-gated
input double InpNewestLegProfitBeforeAddMoney   = 0.0;                 // Legacy input; unused in v4.00
...
input int    InpMinimumWarmupSeconds             = 30;
...
input double InpStopLossATR                     = 1.20;                 // Initial/provisional order protection before direction is confirmed
input double InpTakeProfitATR                   = 0.80;                 // Every position receives a broker-side TP
...
input double InpProvisionalLockGivebackPercent  = 35.0;                 // Retain about 65% of Position 1's best floating profit
input int    InpProtectionExtraBufferPoints     = 12;                   // Extra distance beyond broker stops/freeze for profit-lock modifications
input int    InpProtectionRetryMilliseconds     = 100;                  // Broker-request retry pacing only; never a strategy cooldown
input double InpProtectionFallbackMinimumNet    = 0.01;                 // Smallest net profit worth protecting with a fallback legal SL
input double InpBreakEvenTriggerATR             = 0.30;                 // Move each position to cost-adjusted BE
input double InpTrailingActivationATR           = 0.45;                 // Start individual trailing after stronger progress
input double InpNewestTrailATR                  = 0.25;                 // Individual trailing distance
input double InpMiddleTrailATR                  = 0.10;                 // Legacy input; unused in v4.00
input double InpOldestTrailATR                  = 0.16;                 // Legacy input; unused in v4.00


input group "Scout proof and re-acceleration"
input double InpScoutStallPeakMoney             = 0.25;                 // Stall exit activates only after the scout has shown useful profit
input double InpScoutStallMinimumNetMoney       = 0.05;                 // Never call a stall exit unless at least this net profit remains
input int    InpScoutStallConfirmMilliseconds   = 800;                  // Short confirmation that live momentum has actually stalled
input double InpScoutSupportVelocity1ATR        = 0.008;                // Minimum same-direction 1-second support while the scout runs
input double InpScoutSupportVelocity3ATR        = 0.004;                // Minimum same-direction 3-second support while the scout runs
input double InpConfirmationMinimumPeakMoney    = 0.60;                 // Position 2 is considered only after the scout has proved itself
input double InpConfirmationMinimumCurrentNet   = 0.35;                 // Scout must still retain useful profit before Position 2 is armed
input double InpConfirmationVelocity1ATR        = 0.018;                // Same-direction re-acceleration needed for Position 2
input double InpConfirmationVelocity3ATR        = 0.010;
input double InpConfirmationAcceleration        = 1.05;
input double InpConfirmationTickExpansion       = 1.05;
input bool   InpConfirmationRequireMicroBreak   = true;
input bool   InpConfirmationRespectM5Bias       = true;                 // Do not add size directly against the current M5 EMA direction
input int    InpConfirmationPendingLifetimeSec  = 8;                    // Stale continuation orders are cancelled and rebuilt only on fresh proof
input double InpLadderMinimumBasketProfitMoney  = 0.40;                 // A confirmed basket adds only while already profitable
...
// v4.00 tick-burst, provisional-lock, ladder and shared-stop state.
...
bool burst_reset_ready = true;
bool burst_bracket_armed = false;
ulong burst_bracket_armed_ms = 0;
string burst_arm_reason = "WAITING FOR LIVE BURST";
string burst_arm_side = "NONE";
...
bool urgent_scout_exit_pending = false;
ulong urgent_scout_exit_ticket = 0;
ulong urgent_scout_last_request_ms = 0;
string urgent_scout_exit_reason = "";
ulong scout_stall_started_ms = 0;
ulong confirmation_armed_ms = 0;
string scout_proof_state = "WAITING FOR DIRECTIONAL BURST";

enum ENUM_EXECUTION_SUPERVISOR_STATE
{
   SUPERVISOR_BOOT = 0,
   SUPERVISOR_IDLE = 1,
   SUPERVISOR_ARMED = 2,
   SUPERVISOR_SCOUT = 3,
   SUPERVISOR_CONFIRMED = 4,
   SUPERVISOR_EXITING = 5,
   SUPERVISOR_RECOVERY = 6,
   SUPERVISOR_FAULT = 7
};

ENUM_EXECUTION_SUPERVISOR_STATE supervisor_state = SUPERVISOR_BOOT;
bool supervisor_recovery_latched = false;
string supervisor_recovery_reason = "BOOTING";
ulong supervisor_recovery_ticket = 0;
ulong supervisor_last_recovery_request_ms = 0;
int supervisor_recovery_attempts = 0;
...
int OnInit()
{
   ...
   if(trade_symbol == "")
   {
      Print("EVE Momentum v4.00: could not resolve a trading symbol.");
      return INIT_FAILED;
   }
   if(!SymbolSelect(trade_symbol, true))
   {
      Print("EVE Momentum v4.00: cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   trade.SetMarginMode();
   // Synchronous request mode is intentional. The strict supervisor validates the
   // resulting live MT5 state after every accepted/rejected request before allowing
   ...
   {
      Print("EVE Momentum v4.00: indicator handle creation failed. Error ", GetLastError());
      return INIT_FAILED;
   }
   ...
   CreatePanel();
   EventSetTimer(1);
   last_event = "EA started; collecting live ticks for acceleration-based burst arming";
   SendHeartbeat();
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result)
{
   ...
   if(entry == DEAL_ENTRY_IN || entry == DEAL_ENTRY_INOUT)
   {
      ...
      if(first_leg)
      {
         burst_bracket_armed = false;
         burst_bracket_armed_ms = 0;
         burst_arm_side = "NONE";
      }
      ...
      double planned_entry = HistoryOrderPlannedEntry(order_ticket);
      if(fill_breach == "" && planned_entry > 0.0)
      {
         double point = SymbolInfoDouble(trade_symbol, SYMBOL_POINT);
         double allowed = MathMax(MathMax(0, InpMaximumFirstFillSlippagePoints) * point,
                                  CurrentOrderAtr() * MathMax(0.0, InpMaximumFirstFillSlippageATR));
         double adverse = execution_side == "BUY" ? price - planned_entry : planned_entry - price;
         if(adverse > allowed + 0.0000001)
            fill_breach = StringFormat("FIRST FILL SLIPPAGE BREACH: planned %.2f, filled %.2f, adverse %.2f > %.2f", planned_entry, price, adverse, allowed);
      }
      MqlTick fill_tick;
      ...
      if(first_leg)
      {
         ...
         campaign_phase = CAMPAIGN_OCO_CANCELLING; // v4.00: protected directional scout
         scout_stall_started_ms = 0;
         confirmation_armed_ms = 0;
         scout_proof_state = "SCOUT OPEN - PROTECTING FIRST POSITION";
         basket_entry_atr = CurrentOrderAtr();
         ...
         basket_entry_reason = "DIRECTIONAL TICK-BURST SCOUT TRIGGER";
         baskets_today++;
         pending_sync_state = "SCOUT OPEN - CANCELLING ALL OTHER PENDING ORDERS";
         last_event = StringFormat("%s directional scout triggered at %.2f; no opposite trade will be opened inside this campaign", execution_side, price);
      }
         ...
         if(campaign_phase == CAMPAIGN_OCO_CANCELLING)
         {
            if(execution_side == campaign_current_side)
            {
               ...
               if(same_positions >= 2)
               {
                  ...
                  last_event = StringFormat("Second %s stop triggered at %.2f; protected re-acceleration confirmed and shared-SL ladder management is now active", execution_side, price);
            ...
            {
               execution_integrity_breach = true;
               execution_integrity_reason = StringFormat("OPPOSITE %s FILL DURING %s SCOUT - NO REVERSAL CAMPAIGNS ARE ALLOWED", execution_side, campaign_current_side);
               execution_integrity_position_id = position_id;
               campaign_phase = CAMPAIGN_CLOSING;
               pending_sync_state = "OPPOSITE FILL SAFETY - BANKING FULL BASKET";
               last_event = execution_integrity_reason;
               SendEvent("opposite-fill-safety", last_event);
            }
         }
         else if(campaign_phase == CAMPAIGN_ACTIVE && execution_side != campaign_current_side)
         {
            execution_integrity_breach = true;
            execution_integrity_reason = StringFormat("LATE OPPOSITE %s FILL DURING CONFIRMED %s BASKET", execution_side, campaign_current_side);
            execution_integrity_position_id = position_id;
            campaign_phase = CAMPAIGN_CLOSING;
            pending_sync_state = "OPPOSITE FILL SAFETY - BANKING FULL BASKET";
            last_event = execution_integrity_reason;
      ...
      if(execution_side == campaign_current_side && !close_pending && campaign_phase != CAMPAIGN_CLOSING)
      {
         ...
         shared_stop_state = "NEW FILL - RECALCULATING INDIVIDUAL SL/TP LADDER";
   ...
   if(entry == DEAL_ENTRY_OUT || entry == DEAL_ENTRY_OUT_BY || entry == DEAL_ENTRY_INOUT)
   {
      ...
      {
         pending_sync_state = "INDIVIDUAL POSITION SL HIT";
         last_event = StringFormat("%s position %I64u closed at its own SL; remaining campaign continues only while momentum remains", position_side, position_id);
         SendEvent("individual-sl", last_event);
      }
...
double HistoryOrderPlannedEntry(ulong order_ticket)
{
   if(order_ticket == 0 || !HistoryOrderSelect(order_ticket)) return 0.0;
   return HistoryOrderGetDouble(order_ticket, ORDER_PRICE_OPEN);
}
...
string CampaignPhaseText()
{
   ...
   if(campaign_phase == CAMPAIGN_BUILDING_STRADDLE) return "ARMING DIRECTIONAL SCOUT";
   if(campaign_phase == CAMPAIGN_STRADDLE_READY) return "SCOUT STOP READY";
   if(campaign_phase == CAMPAIGN_OCO_CANCELLING) return "SCOUT ACTIVE";
   ...
   if(campaign_phase == CAMPAIGN_WAIT_NEXT_CANDLE) return "WAIT QUIET RESET";
   return "UNKNOWN";
}
...
bool DeleteOneForFlatStraddle()
{
   if(!InpDirectionalScoutOnly)
   {
      ...
      for(int i=OrdersTotal()-1; i>=0; i--)
      {
         ...
         ENUM_ORDER_TYPE type = (ENUM_ORDER_TYPE)OrderGetInteger(ORDER_TYPE);
         string role = SelectedPendingRole();
         if(role == "INITIAL" && type == ORDER_TYPE_BUY_STOP && !kept_buy)
         {
            kept_buy = true;
            continue;
         }
         if(role == "INITIAL" && type == ORDER_TYPE_SELL_STOP && !kept_sell)
         {
            kept_sell = true;
            continue;
         }
         DeleteSelectedPending("research bracket keeps at most one initial stop on each side");
         return true;
      }
      return false;
   }

   if(burst_arm_side != "BUY" && burst_arm_side != "SELL")
   {
      if(CountOurPendingOrders() > 0) CancelPendingOrders();
      return CountOurPendingOrders() > 0;
   }
   ENUM_ORDER_TYPE wanted = StopTypeForSide(burst_arm_side);
   bool kept_one = false;
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ulong ticket = OrderGetTicket(i);
      ...
      ENUM_ORDER_TYPE type = (ENUM_ORDER_TYPE)OrderGetInteger(ORDER_TYPE);
      string role = SelectedPendingRole();
      if(role == "INITIAL" && type == wanted && !kept_one)
      {
         kept_one = true;
         continue;
      }
      DeleteSelectedPending("directional scout keeps only the detected burst-side stop");
      return true;
   }
   return false;
}
...
bool DeleteOneForProvisional(string side)
{
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      ENUM_ORDER_TYPE type = (ENUM_ORDER_TYPE)OrderGetInteger(ORDER_TYPE);
      string role = SelectedPendingRole();
      if(type == same_type && role == "CONFIRMATION" && !kept_confirmation)
      {
         kept_confirmation = true;
         continue;
      }
      DeleteSelectedPending("scout campaign cancels every opposite or stale pending order");
      return true;
   }
   return false;
}
...
double RoundStopAwayFromMarket(string side, double price)
{
   double tick_size = SymbolInfoDouble(trade_symbol, SYMBOL_TRADE_TICK_SIZE);
   if(tick_size <= 0.0) tick_size = SymbolInfoDouble(trade_symbol, SYMBOL_POINT);
   if(tick_size <= 0.0) return NormalizePrice(price);
...
double BrokerLegalProtectionBoundary(string side, int extra_buffer_points)
{
   MqlTick tick;
   if(!SymbolInfoTick(trade_symbol, tick)) return 0.0;
   double point = SymbolInfoDouble(trade_symbol, SYMBOL_POINT);
   ...
   double distance = MathMax(BrokerStopsDistancePrice(), BrokerFreezeDistancePrice());
   distance += MathMax(2.0 * tick_size, MathMax(0, extra_buffer_points) * point);
   double raw = side == "BUY" ? tick.bid - distance : tick.ask + distance;
   return RoundStopAwayFromMarket(side, raw);
}

bool NetProfitAtClosePrice(string side, double entry, double volume, double close_price, double &net_profit)
{
   net_profit = 0.0;
   ENUM_ORDER_TYPE calc_type = side == "BUY" ? ORDER_TYPE_BUY : ORDER_TYPE_SELL;
   double gross = 0.0;
   if(!OrderCalcProfit(calc_type, trade_symbol, volume, entry, close_price, gross)) return false;
   double commission_reserve = MathMax(0.0, InpSharedSLCommissionReservePerLot) * volume;
   net_profit = gross - commission_reserve;
   return true;
}

bool ResolveLegalProfitLockTarget(string side, double desired_target, double entry, double volume, int extra_buffer_points, double &legal_target, double &protected_net, string &reason)
{
   legal_target = 0.0;
   protected_net = 0.0;
   reason = "";
   double boundary = BrokerLegalProtectionBoundary(side, extra_buffer_points);
   if(boundary <= 0.0)
   {
      reason = "No live broker protection boundary is available";
      return false;
   }

   legal_target = side == "BUY" ? MathMin(desired_target, boundary) : MathMax(desired_target, boundary);
   legal_target = RoundStopAwayFromMarket(side, legal_target);
   bool profitable_side = side == "BUY" ? legal_target > entry : legal_target < entry;
   if(!profitable_side)
   {
      reason = StringFormat("Nearest broker-legal SL %.2f cannot protect profit above/below entry %.2f", legal_target, entry);
      return false;
   }
   if(!NetProfitAtClosePrice(side, entry, volume, legal_target, protected_net))
   {
      reason = "Could not calculate fallback SL profit";
      return false;
   }
   if(protected_net + 0.0001 < MathMax(0.0, InpProtectionFallbackMinimumNet))
   {
      reason = StringFormat("Nearest broker-legal SL %.2f protects only $%.2f net", legal_target, protected_net);
      return false;
   }
   return true;
}

bool ProtectionRequestAvailable()
{
   if(!TerminalInfoInteger(TERMINAL_CONNECTED)) return false;
   if(!TerminalInfoInteger(TERMINAL_TRADE_ALLOWED)) return false;
   if(!MQLInfoInteger(MQL_TRADE_ALLOWED)) return false;
   ulong now_ms = GetTickCount64();
   if(urgent_scout_last_request_ms > 0 && now_ms - urgent_scout_last_request_ms < (ulong)MathMax(0, InpProtectionRetryMilliseconds)) return false;
   return true;
}

void ArmUrgentScoutCloseState(string reason, double trigger_profit)
{
   ...
   last_close_result = "Urgent scout close requested before pending-order cleanup";
}

bool ContinueUrgentScoutExit()
{
   if(!urgent_scout_exit_pending) return false;

   if(urgent_scout_exit_ticket == 0 || !PositionSelectByTicket(urgent_scout_exit_ticket) || !IsOurSelectedPosition())
   {
      urgent_scout_exit_pending = false;
      urgent_scout_exit_ticket = 0;
      urgent_scout_last_request_ms = 0;
      pending_sync_state = "SCOUT EXIT CONFIRMED - CLEANING PENDING ORDERS";
      last_event = pending_sync_state;
      ContinuePendingClose();
      return true;
   }

   engine_state = "URGENT SCOUT PROFIT EXIT";
   pending_sync_state = "PROFIT LOCK UNAVAILABLE - CLOSING POSITION BEFORE PENDING CLEANUP";
   if(!ProtectionRequestAvailable()) return true;

   urgent_scout_last_request_ms = GetTickCount64();
   close_attempts++;
   last_close_attempt_at = TimeCurrent();
   ResetLastError();
   bool submitted = trade.PositionClose(urgent_scout_exit_ticket, InpSlippagePoints);
   uint code = trade.ResultRetcode();
   last_close_retcode = code;
   last_close_result = trade.ResultRetcodeDescription();
   if(!submitted || !CloseResultAccepted(code))
   {
      provisional_profit_lock_state = StringFormat("URGENT SCOUT CLOSE RETRY: %u %s", code, trade.ResultRetcodeDescription());
      pending_sync_state = provisional_profit_lock_state;
      last_event = provisional_profit_lock_state;
      return true;
   }

   provisional_profit_lock_state = "URGENT SCOUT CLOSE ACCEPTED - WAITING BROKER CONFIRMATION";
   pending_sync_state = provisional_profit_lock_state;
   last_event = provisional_profit_lock_state;
   return true;
}

bool StartUrgentScoutExit(ulong ticket, string reason)
{
   if(!urgent_scout_exit_pending)
   {
      urgent_scout_exit_pending = true;
      urgent_scout_exit_ticket = ticket;
      urgent_scout_exit_reason = reason;
      urgent_scout_last_request_ms = 0;
      SendBankDecision(reason, BasketProfit(), 0, 0);
      ArmUrgentScoutCloseState(reason, BasketProfit());
      provisional_profit_lock_state = reason;
      last_event = reason;
      SendEvent("protection-fallback", reason);
   }
   return ContinueUrgentScoutExit();
}

bool MaintainProvisionalProfitLock()
{
   string side = campaign_current_side;
   if(side != "BUY" && side != "SELL") side = BasketSideText();
   if(side != "BUY" && side != "SELL" || CountOurPositionsBySide(side) != 1) return false;

   if(urgent_scout_exit_pending) return ContinueUrgentScoutExit();
   ...
   if(!already_profit_locked && estimated_current_net <= MathMax(0.0, InpProtectionFallbackMinimumNet) + 0.0001)
   {
      string exit_reason = StringFormat("POSITION 1 PROFIT PROTECTION FALLBACK - current net $%.2f; closing before full loss", estimated_current_net);
      return StartUrgentScoutExit(ticket, exit_reason);
   }

   double desired_target = 0.0;
   string reason = "";
   if(!CalculateProvisionalLockTarget(side, required_gross, desired_target, reason))
   {
      if(already_profit_locked)
      {
         double existing_net = 0.0;
         NetProfitAtClosePrice(side, entry, volume, existing_sl, existing_net);
         provisional_profit_lock_stop = existing_sl;
         provisional_profit_locked_money = existing_net;
         provisional_profit_lock_state = StringFormat("POSITION 1 EXISTING PROFIT LOCK HOLDS: SL %.2f protects about $%.2f net", existing_sl, existing_net);
         return false;
      }
      string exit_reason = "POSITION 1 PROFIT LOCK CALCULATION FAILED - " + reason + "; closing scout now";
      return StartUrgentScoutExit(ticket, exit_reason);
   }

   double target = 0.0;
   double protected_net = 0.0;
   if(!ResolveLegalProfitLockTarget(side, desired_target, entry, volume, InpProtectionExtraBufferPoints, target, protected_net, reason))
   {
      if(already_profit_locked)
      {
         double existing_net = 0.0;
      ...
      string exit_reason = "NO BROKER-LEGAL PROFIT SL - " + reason + "; closing scout now";
      return StartUrgentScoutExit(ticket, exit_reason);
   }
   ...
   if(existing_is_stronger)
   {
      provisional_profit_lock_stop = existing_sl;
      double existing_net = 0.0;
      if(!NetProfitAtClosePrice(side, entry, volume, existing_sl, existing_net)) existing_net = protected_net;
      provisional_profit_locked_money = existing_net;
      provisional_profit_lock_state = StringFormat("POSITION 1 PROFIT LOCKED: SL %.2f protects about $%.2f net", existing_sl, existing_net);
      return false;
   }

   if(!ProtectionRequestAvailable())
   {
      provisional_profit_lock_state = "POSITION 1 PROTECTION HAS REQUEST PRIORITY - waiting for broker request channel";
      pending_sync_state = provisional_profit_lock_state;
      return true;
   }
   ...
   {
      RegisterPendingFailure(code);
      provisional_profit_lock_state = StringFormat("Position 1 legal SL %.2f rejected: %u %s", target, code, trade.ResultRetcodeDescription());
      pending_sync_state = provisional_profit_lock_state;
      last_event = provisional_profit_lock_state;

      // Price can move between calculation and broker validation. Recalculate once with
      // a larger distance, then close immediately if the broker still refuses protection.
      int retry_buffer = InpProtectionExtraBufferPoints + MathMax(10, pending_invalid_price_streak * 4);
      double retry_target = 0.0;
      double retry_net = 0.0;
      string retry_reason = "";
      bool retry_possible = ResolveLegalProfitLockTarget(side, desired_target, entry, volume, retry_buffer, retry_target, retry_net, retry_reason);
      bool retry_different = retry_possible && MathAbs(retry_target - target) >= SymbolInfoDouble(trade_symbol, SYMBOL_TRADE_TICK_SIZE) * 0.5;
      if(retry_different)
      {
         urgent_scout_last_request_ms = GetTickCount64();
         ResetLastError();
         bool retry_submitted = trade.PositionModify(ticket, retry_target, tp);
         uint retry_code = trade.ResultRetcode();
         if(retry_submitted && TradeResultAccepted(retry_code))
         {
            provisional_profit_lock_stop = retry_target;
            provisional_profit_locked_money = retry_net;
            provisional_profit_lock_state = StringFormat("POSITION 1 FALLBACK PROFIT LOCK ARMED: SL %.2f protects about $%.2f net", retry_target, retry_net);
            pending_sync_state = provisional_profit_lock_state;
            last_event = provisional_profit_lock_state;
            SendEvent("provisional-lock", last_event);
            return true;
         }
         RegisterPendingFailure(retry_code);
         provisional_profit_lock_state = StringFormat("Fallback SL %.2f rejected: %u %s", retry_target, retry_code, trade.ResultRetcodeDescription());
         pending_sync_state = provisional_profit_lock_state;
         last_event = provisional_profit_lock_state;
      }

      string exit_reason = "BROKER REJECTED PROFIT PROTECTION - closing scout before pending cleanup";
      return StartUrgentScoutExit(ticket, exit_reason);
   }

   provisional_profit_lock_stop = target;
   provisional_profit_locked_money = protected_net;
   provisional_profit_lock_state = StringFormat("POSITION 1 PROFIT LOCK ARMED: SL %.2f protects about $%.2f net", target, protected_net);
   pending_sync_state = provisional_profit_lock_state;
   last_event = provisional_profit_lock_state;
   SendEvent("provisional-lock", last_event);
   return true;
...
int PendingLifetimeSecondsForRole(string role)
{
   if(role == "INITIAL" || role == "ENTRY-BRACKET") return MathMax(3, InpBurstBracketLifetimeSeconds);
   if(role == "CONFIRMATION" || role == "REVERSAL") return MathMax(3, InpConfirmationPendingLifetimeSec);
   if(role == "LADDER") return MathMax(3, InpConfirmationPendingLifetimeSec);
   return MathMax(3, InpBurstBracketLifetimeSeconds);
}

bool BrokerSupportsSpecifiedExpiration()
{
   long modes = SymbolInfoInteger(trade_symbol, SYMBOL_EXPIRATION_MODE);
   return (modes & SYMBOL_EXPIRATION_SPECIFIED) == SYMBOL_EXPIRATION_SPECIFIED;
}

ENUM_ORDER_TYPE_TIME PendingTimePolicy(string role, datetime &expiration)
{
   expiration = 0;
   if(InpUseServerPendingExpiration && BrokerSupportsSpecifiedExpiration())
   {
      expiration = TimeCurrent() + PendingLifetimeSecondsForRole(role);
...
bool PlacePendingOrderExact(ENUM_ORDER_TYPE type, string role, double volume, double entry, double sl, string reason)
{
   ...
   double tp_distance = MathMax(CurrentOrderAtr() * MathMax(0.10, InpTakeProfitATR), BrokerStopsDistancePrice() + PendingAdaptiveBufferPrice());
   double tp = InpTakeProfitATR > 0.0 ? NormalizePrice(side == "BUY" ? entry + tp_distance : entry - tp_distance) : 0.0;
   ...
   string comment = "EVE33-LAD";
   if(role == "INITIAL" || role == "ENTRY-BRACKET") comment = "EVE33-INIT";
   else if(role == "CONFIRMATION" || role == "REVERSAL") comment = "EVE33-CONF";

   datetime expiration = 0;
   ENUM_ORDER_TYPE_TIME time_policy = PendingTimePolicy(role, expiration);
   MarkTradeRequest();
   bool submitted = type == ORDER_TYPE_BUY_STOP
      ? trade.BuyStop(NormalizeVolume(volume), entry, trade_symbol, sl, tp, time_policy, expiration, comment)
      : trade.SellStop(NormalizeVolume(volume), entry, trade_symbol, sl, tp, time_policy, expiration, comment);
   uint code = trade.ResultRetcode();
   if((!submitted || !TradeResultAccepted(code)) && time_policy == ORDER_TIME_SPECIFIED && code == TRADE_RETCODE_INVALID_EXPIRATION)
   {
      // Some brokers advertise specified expiry but reject short lifetimes. Retry the
      // same broker-safe order as GTC; the supervisor still enforces local age.
      MarkTradeRequest();
      submitted = type == ORDER_TYPE_BUY_STOP
         ? trade.BuyStop(NormalizeVolume(volume), entry, trade_symbol, sl, tp, ORDER_TIME_GTC, 0, comment)
         : trade.SellStop(NormalizeVolume(volume), entry, trade_symbol, sl, tp, ORDER_TIME_GTC, 0, comment);
   ...
   SendOrderActivity("PLACED", role, type, ticket, volume, entry, reason + StringFormat(" | INDIVIDUAL SL %.2f | TP %.2f", sl, tp));
   return true;
}


double EstimatedScoutNetProfit()
{
   UpdateNewestCanary();
   double volume = 0.0;
   if(newest_ticket > 0 && PositionSelectByTicket(newest_ticket) && IsOurSelectedPosition())
      volume = PositionGetDouble(POSITION_VOLUME);
   double reserve = MathMax(0.0, InpSharedSLCommissionReservePerLot) * volume;
   return newest_leg_current_profit - reserve;
}

bool ScoutHasBrokerProfitLock(string side)
{
   ulong ticket = 0;
   double entry = 0.0, volume = 0.0, sl = 0.0, tp = 0.0;
   if(!FindSinglePositionBySide(side, ticket, entry, volume, sl, tp)) return false;
   if(side == "BUY") return sl > entry;
   if(side == "SELL") return sl > 0.0 && sl < entry;
   return false;
}

bool LiveReaccelerationReady(string side, string &reason)
{
   reason = "";
   if(!have_scan)
   {
      reason = "waiting for live momentum scan";
      return false;
   }

   bool direction_ok = side == "BUY"
      ? (last_scan.velocity_1s >= InpConfirmationVelocity1ATR && last_scan.velocity_3s >= InpConfirmationVelocity3ATR)
      : (last_scan.velocity_1s <= -InpConfirmationVelocity1ATR && last_scan.velocity_3s <= -InpConfirmationVelocity3ATR);
   if(!direction_ok)
   {
      reason = StringFormat("waiting re-acceleration: v1 %.3f / v3 %.3f", last_scan.velocity_1s, last_scan.velocity_3s);
      return false;
   }

   if(last_scan.acceleration < InpConfirmationAcceleration)
   {
      reason = StringFormat("waiting acceleration x%.2f / x%.2f", last_scan.acceleration, InpConfirmationAcceleration);
      return false;
   }
   if(last_scan.tick_rate_ratio < InpConfirmationTickExpansion)
   {
      reason = StringFormat("waiting tick expansion x%.2f / x%.2f", last_scan.tick_rate_ratio, InpConfirmationTickExpansion);
      return false;
   }

   bool micro_break = side == "BUY" ? last_scan.micro_break_buy : last_scan.micro_break_sell;
   if(InpConfirmationRequireMicroBreak && !micro_break)
   {
      reason = "waiting fresh same-direction micro-break";
      return false;
   }

   if(InpConfirmationRespectM5Bias)
   {
      string opposite = OppositeSide(side);
      if(last_scan.m5_confirmation == opposite)
      {
         reason = "M5 is directly opposite; scout can bank but size will not be added";
         return false;
      }
   }
...
bool ScoutReadyForConfirmation(string side, string &reason)
{
   reason = "";
   UpdateNewestCanary();
   double current_net = EstimatedScoutNetProfit();
   if(newest_leg_peak_profit + 0.0001 < InpConfirmationMinimumPeakMoney)
   {
      reason = StringFormat("scout peak $%.2f / $%.2f proof", newest_leg_peak_profit, InpConfirmationMinimumPeakMoney);
      return false;
   }
   if(current_net + 0.0001 < InpConfirmationMinimumCurrentNet)
   {
      reason = StringFormat("scout current net $%.2f / $%.2f proof", current_net, InpConfirmationMinimumCurrentNet);
      return false;
   }
   if(!ScoutHasBrokerProfitLock(side))
   {
      reason = "scout has not yet secured a broker-side profit lock";
      return false;
   }
   return LiveReaccelerationReady(side, reason);
}

bool MaintainScoutStallExit()
{
   string side = campaign_current_side;
   if(side != "BUY" && side != "SELL" || CountOurPositionsBySide(side) != 1)
   {
      scout_stall_started_ms = 0;
      return false;
   }
   ...
   UpdateNewestCanary();
   double current_net = EstimatedScoutNetProfit();
   if(newest_leg_peak_profit + 0.0001 < InpScoutStallPeakMoney || current_net + 0.0001 < InpScoutStallMinimumNetMoney)
   {
      scout_stall_started_ms = 0;
      return false;
   }

   bool supported = have_scan && (side == "BUY"
      ? (last_scan.velocity_1s >= InpScoutSupportVelocity1ATR && last_scan.velocity_3s >= InpScoutSupportVelocity3ATR)
      : (last_scan.velocity_1s <= -InpScoutSupportVelocity1ATR && last_scan.velocity_3s <= -InpScoutSupportVelocity3ATR));
   if(supported)
   {
      scout_stall_started_ms = 0;
      scout_proof_state = StringFormat("SCOUT RUNNING: peak $%.2f / current net $%.2f", newest_leg_peak_profit, current_net);
      return false;
   }

   ulong now_ms = GetTickCount64();
   if(scout_stall_started_ms == 0)
   {
      scout_stall_started_ms = now_ms;
      scout_proof_state = StringFormat("SCOUT STALL WATCH: protecting $%.2f net", current_net);
      return false;
   }

   ulong needed = (ulong)MathMax(0, InpScoutStallConfirmMilliseconds);
   if(now_ms - scout_stall_started_ms < needed) return false;

   scout_proof_state = StringFormat("SCOUT MOMENTUM STALLED: banking $%.2f net before deeper giveback", current_net);
   BankBasket("SCOUT MOMENTUM STALLED - BANK PROFIT", BasketProfit());
   return true;
}

bool BasketReadyForNextLadder(string side, string &reason)
{
   reason = "";
   double current = BasketProfit();
   if(current + 0.0001 < InpLadderMinimumBasketProfitMoney)
   {
      reason = StringFormat("basket profit $%.2f / $%.2f required before next leg", current, InpLadderMinimumBasketProfitMoney);
      return false;
   }
   return LiveReaccelerationReady(side, reason);
}
...
void MaintainProvisionalCampaign()
{
   ...
   if(runtime_maximum_positions>0 && positions>=runtime_maximum_positions){ CancelPendingOrders(); pending_sync_state="MAX POSITIONS REACHED"; return; }
   string spread_reason="";
   if(!EntrySpreadSafe(spread_reason)){ CancelPendingOrders(); pending_sync_state=spread_reason; return; }
   string momentum_reason="";
   if(!LiveReaccelerationReady(side,momentum_reason)){ CancelPendingOrders(); pending_sync_state="WAITING SAME-DIRECTION MOMENTUM: "+momentum_reason; return; }
   if(CountPendingBySideAndRole(side,"CONFIRMATION")>0) return;
   ...
   PlacePendingOrder(StopTypeForSide(side),"CONFIRMATION",CalculateLegLot(),desired,CurrentOrderAtr(),0.0,"rapid continuation order with individual SL/TP");
}
...
void MaintainActiveLadder()
{
   string side=campaign_current_side;
   if(side!="BUY" && side!="SELL") return;
   if(DeleteOneForActiveLadder(side)) return;
   int positions=CountOurPositions();
   if(runtime_maximum_positions>0 && positions>=runtime_maximum_positions){ CancelPendingOrders(); pending_sync_state="MAX POSITIONS REACHED"; return; }
   string spread_reason="";
   if(!EntrySpreadSafe(spread_reason)){ CancelPendingOrders(); pending_sync_state=spread_reason; return; }
   string momentum_reason="";
   if(!LiveReaccelerationReady(side,momentum_reason)){ CancelPendingOrders(); pending_sync_state="MOMENTUM FADED - NO NEW LEG"; return; }
   if(CountPendingBySideAndRole(side,"LADDER")>0) return;
   double base=last_add_price>0.0?last_add_price:AverageBasketEntry();
   double desired=side=="BUY"?base+LadderSpacingPrice(side):base-LadderSpacingPrice(side);
   PlacePendingOrder(StopTypeForSide(side),"LADDER",CalculateLegLot(),desired,CurrentOrderAtr(),0.0,"one rapid ladder stop ahead; every leg has its own SL/TP");
}
...
bool BuildLiveScan(const MqlTick &tick, MomentumSnapshot &scan)
{
   ...
   double speed1 = MathAbs(v1);
   double speed3 = MathAbs(v3);
   double acceleration = speed3 > 0.0000001 ? speed1 / speed3 : (speed1 > 0.0 ? 99.0 : 0.0);
...
{
   ResetDailyIfNeeded();
   ManageIndividualStopsAndTargets();
   ...
   if(CountOurPositions() > 0)
   {
      scan.block_reason = campaign_phase == CAMPAIGN_OCO_CANCELLING
         ? "DIRECTIONAL SCOUT - PROFIT LOCK / STALL EXIT / RE-ACCELERATION PROOF"
         : "CONFIRMED CAMPAIGN - SHARED SL; NEXT LEG ONLY ON FRESH RE-ACCELERATION";
      return;
   }

   if(!burst_reset_ready && !burst_bracket_armed)
   {
      scan.block_reason = "WAITING FOR PRICE TO QUIETEN BEFORE THE NEXT GENUINE BURST";
      return;
   }
   ...
   scan.block_reason = burst_bracket_armed ? "LIVE BURST DETECTED - DIRECTIONAL SCOUT STOP ARMED" : "WATCHING LIVE TICKS FOR ACCELERATION";
}


bool OpenMarketLeg(ENUM_POSITION_TYPE direction, double volume, double atr_value)
{
   // v4.00 never adds with market orders. This helper remains only for backward compatibility.
   return false;
}


bool IsLegacyMagic(ulong magic)
{
   return magic == InpLegacyMagicNumber || magic == InpOlderLegacyMagicNumber || magic == InpOldestLegacyMagicNumber || magic == InpAncientLegacyMagicNumber;
}

bool IsLegacyEveComment(string comment)
{
   return StringFind(comment, "EVE31-") >= 0 || StringFind(comment, "EVE3-") >= 0 || StringFind(comment, "EVE-MOMENTUM-V3.00") >= 0 ||
      StringFind(comment, "EVE-MOMENTUM-V2.11") >= 0 ||
      StringFind(comment, "EVE30-") >= 0 || StringFind(comment, "EVE-MOMENTUM-V2.10") >= 0 ||
      StringFind(comment, "EVE29-") >= 0 || StringFind(comment, "EVE-MOMENTUM-V2.09") >= 0 ||
...
{
   if(!InpDeleteLegacyPendingOrders) return 0;
   int count = 0;
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ulong ticket = OrderGetTicket(i);
      ...
      if(!IsLegacyMagic((ulong)OrderGetInteger(ORDER_MAGIC))) continue;
...
{
   if(InpLegacyMagicNumber == InpMagicNumber && InpOlderLegacyMagicNumber == InpMagicNumber && InpOldestLegacyMagicNumber == InpMagicNumber) return 0;
   int count = 0;
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ulong ticket = PositionGetTicket(i);
      ...
      if(!IsLegacyMagic((ulong)PositionGetInteger(POSITION_MAGIC))) continue;
...
bool HandleLegacyPendingCleanup()
{
   ...
   last_event = "v4.00 is deleting recognised older EVE pending orders before starting";
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      delete_wait_ticket = ticket;
      delete_wait_started_ms = GetTickCount64();
      SendOrderActivity("LEGACY-CANCEL-REQUESTED", "LEGACY", type, ticket, volume, price, "v4.00 version isolation cleanup");
      return true;
   }
   return true;
}
...
string SupervisorStateText()
{
   if(supervisor_state == SUPERVISOR_BOOT) return "BOOT";
   if(supervisor_state == SUPERVISOR_IDLE) return "IDLE";
   if(supervisor_state == SUPERVISOR_ARMED) return "ARMED";
   if(supervisor_state == SUPERVISOR_SCOUT) return "SCOUT";
   if(supervisor_state == SUPERVISOR_CONFIRMED) return "CONFIRMED";
   if(supervisor_state == SUPERVISOR_EXITING) return "EXITING";
   if(supervisor_state == SUPERVISOR_RECOVERY) return "RECOVERY";
   return "FAULT";
}

bool PendingRoleAllowedForReality(string role, string side, int positions, string position_side)
{
   if(positions == 0) return role == "INITIAL";
   if(side != position_side) return false;
   if(positions == 1) return role == "CONFIRMATION";
   return role == "LADDER";
}

bool DeleteSelectedPendingRecovery(string reason)
{
   ulong ticket = (ulong)OrderGetInteger(ORDER_TICKET);
   ENUM_ORDER_TYPE type = (ENUM_ORDER_TYPE)OrderGetInteger(ORDER_TYPE);
   double volume = OrderGetDouble(ORDER_VOLUME_CURRENT);
   ...
   ulong now_ms = GetTickCount64();
   if(delete_wait_ticket == ticket && delete_wait_started_ms > 0 &&
      now_ms - delete_wait_started_ms < (ulong)MathMax(1, InpOrderDeleteTimeoutSeconds) * 1000)
      return false;
   if(supervisor_last_recovery_request_ms > 0 && now_ms - supervisor_last_recovery_request_ms < (ulong)MathMax(1, InpWrongSideRetryMilliseconds))
      return false;
   if(!TerminalInfoInteger(TERMINAL_CONNECTED) || !TerminalInfoInteger(TERMINAL_TRADE_ALLOWED) || !MQLInfoInteger(MQL_TRADE_ALLOWED))
      return false;

   supervisor_last_recovery_request_ms = now_ms;
   ...
   {
      delete_wait_ticket = ticket;
      delete_wait_started_ms = now_ms;
      pending_sync_state = "RECOVERY DELETE ACCEPTED";
      SendOrderActivity("RECOVERY-CANCEL", role, type, ticket, volume, price, reason);
      return true;
   }

   // The order may have filled/disappeared between selection and deletion. Rebuild from
   // current MT5 state instead of assuming the old ticket still exists.
   if(!OrderSelect(ticket))
   {
      pending_sync_state = "RECOVERY ORDER CHANGED STATE - RECONCILING";
      return true;
   }

   RegisterPendingFailure(code);
   string failure = StringFormat("%s | delete %I64u rejected: %u %s", reason, ticket, code, trade.ResultRetcodeDescription());
   pending_sync_state = failure;
   last_event = failure;
   SendOrderActivity("RECOVERY-DELETE-FAILED", role, type, ticket, volume, price, failure);
   return false;
}

bool EnforceExecutionInvariants()
{
   if(!InpStrictStateMachine) return false;

   int positions = CountOurPositions();
   int pending = CountOurPendingOrders();
   string position_side = BasketSideText();

   if(close_pending || urgent_scout_exit_pending)
   {
      supervisor_state = SUPERVISOR_EXITING;
      return false;
   }

   if(CountOurPositionsBySide("BUY") > 0 && CountOurPositionsBySide("SELL") > 0)
   {
      supervisor_state = SUPERVISOR_FAULT;
      supervisor_recovery_latched = true;
      supervisor_recovery_reason = "INVARIANT: mixed BUY and SELL positions";
      BankBasket(supervisor_recovery_reason, BasketProfit());
      return true;
   }

   int expected_pending_max = positions == 0 ? 1 : 1;
   int seen_pending = 0;
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ulong ticket = OrderGetTicket(i);
      ...
      string side = PendingSideFromType(type);
      string role = SelectedPendingRole();
      double price = OrderGetDouble(ORDER_PRICE_OPEN);
      datetime setup = (datetime)OrderGetInteger(ORDER_TIME_SETUP);

      bool wrong_side = PendingOrderWrongSide(type, price);
      bool wrong_role = !PendingRoleAllowedForReality(role, side, positions, position_side);
      bool duplicate = seen_pending > expected_pending_max;
      bool transitional_too_long = (order_state == ORDER_STATE_REQUEST_ADD || order_state == ORDER_STATE_REQUEST_MODIFY || order_state == ORDER_STATE_REQUEST_CANCEL || order_state == ORDER_STATE_STARTED) &&
         InpPendingTransitionTimeoutSeconds > 0 && setup > 0 && TimeCurrent() > setup + InpPendingTransitionTimeoutSeconds;
      bool age_expired = setup > 0 && TimeCurrent() > setup + PendingLifetimeSecondsForRole(role) + 2;

      if(wrong_side || wrong_role || duplicate || transitional_too_long || age_expired || side == "NONE" || role == "UNKNOWN")
      {
         supervisor_state = SUPERVISOR_RECOVERY;
         supervisor_recovery_latched = true;
         supervisor_recovery_ticket = ticket;
         string issue = wrong_side ? "WRONG-SIDE PENDING ORDER" :
                        wrong_role ? "PENDING ROLE DOES NOT MATCH LIVE POSITIONS" :
                        duplicate ? "DUPLICATE PENDING ORDER" :
                        transitional_too_long ? "PENDING ORDER STUCK IN TRANSITION" :
                        age_expired ? "STALE PENDING ORDER" : "UNKNOWN PENDING ORDER";
         supervisor_recovery_reason = StringFormat("%s: ticket %I64u %s %s at %.2f", issue, ticket, side, role, price);
         engine_state = "EXECUTION RECOVERY";
         pending_sync_state = supervisor_recovery_reason;
         last_event = supervisor_recovery_reason;
         DeleteSelectedPendingRecovery(supervisor_recovery_reason);
         SavePersistentState();
         return true;
      }
   }

   if(supervisor_recovery_latched)
   {
      if(positions > 0 && !close_pending)
      {
         supervisor_state = SUPERVISOR_FAULT;
         string reason = "RECOVERY ORDER FILLED OR POSITION APPEARED - CLOSE FULL CAMPAIGN: " + supervisor_recovery_reason;
         BankBasket(reason, BasketProfit());
         return true;
      }

      // Once recovery starts, keep removing the selected EVE order even if price later
      // moves back to the technically valid side. Otherwise a rejected cancellation could
      // leave the EA latched in RECOVERY forever with the old order still live.
      if(positions == 0 && pending > 0)
      {
         bool selected_recovery_order = false;
         if(supervisor_recovery_ticket > 0 && OrderSelect(supervisor_recovery_ticket) && IsOurSelectedOrder())
            selected_recovery_order = true;
         if(!selected_recovery_order)
         {
            for(int i=OrdersTotal()-1; i>=0; i--)
            {
               ulong ticket = OrderGetTicket(i);
      ...
      if(InpAutoRecoverWhenFlat && positions == 0 && pending == 0 && !close_pending)
      {
         supervisor_recovery_latched = false;
         supervisor_recovery_reason = "RECOVERED: MT5 flat and order-free";
         supervisor_recovery_ticket = 0;
         supervisor_recovery_attempts = 0;
         supervisor_last_recovery_request_ms = 0;
         pending_sync_state = supervisor_recovery_reason;
         last_event = supervisor_recovery_reason;
      }
      else
   ...
   if(positions == 0 && pending == 0) supervisor_state = SUPERVISOR_IDLE;
   else if(positions == 0 && pending == 1) supervisor_state = SUPERVISOR_ARMED;
   else if(positions == 1) supervisor_state = SUPERVISOR_SCOUT;
   else if(positions >= 2) supervisor_state = SUPERVISOR_CONFIRMED;
   else supervisor_state = SUPERVISOR_FAULT;

   return false;
}

bool ManageIndividualStopsAndTargets()
{
   if(!have_scan || last_scan.atr <= 0.0) return false;
   MqlTick tick;
   if(!SymbolInfoTick(trade_symbol, tick)) return false;
   double atr = last_scan.atr;
   double point = SymbolInfoDouble(trade_symbol, SYMBOL_POINT);
   double legal = BrokerStopsDistancePrice() + BrokerFreezeDistancePrice() + MathMax(2, InpProtectionExtraBufferPoints) * point;
   bool changed = false;
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      double sl=PositionGetDouble(POSITION_SL);
      double tp=PositionGetDouble(POSITION_TP);
      double current=side=="BUY" ? tick.bid : tick.ask;
      double progress=side=="BUY" ? current-entry : entry-current;
      double desired_sl=sl;
      double desired_tp=tp;

      if(desired_tp<=0.0 && InpTakeProfitATR>0.0)
         desired_tp=NormalizePrice(side=="BUY" ? entry+atr*InpTakeProfitATR : entry-atr*InpTakeProfitATR);

      if(progress >= atr*MathMax(0.0,InpBreakEvenTriggerATR))
      {
         double be_buffer=MathMax(2.0*point, CurrentSpreadPoints()*point*0.20);
         double be=NormalizePrice(side=="BUY" ? entry+be_buffer : entry-be_buffer);
         if(side=="BUY") desired_sl=MathMax(desired_sl,be); else desired_sl=(desired_sl<=0.0?be:MathMin(desired_sl,be));
      }

      if(progress >= atr*MathMax(InpBreakEvenTriggerATR,InpTrailingActivationATR))
      {
         double trail=NormalizePrice(side=="BUY" ? current-atr*MathMax(0.05,InpNewestTrailATR) : current+atr*MathMax(0.05,InpNewestTrailATR));
         if(side=="BUY") desired_sl=MathMax(desired_sl,trail); else desired_sl=(desired_sl<=0.0?trail:MathMin(desired_sl,trail));
      }

      if(side=="BUY" && desired_sl>0.0) desired_sl=MathMin(desired_sl,NormalizePrice(tick.bid-legal));
      if(side=="SELL" && desired_sl>0.0) desired_sl=MathMax(desired_sl,NormalizePrice(tick.ask+legal));
      if(side=="BUY" && desired_tp>0.0 && desired_tp<=tick.ask+legal) desired_tp=0.0;
      if(side=="SELL" && desired_tp>0.0 && desired_tp>=tick.bid-legal) desired_tp=0.0;

      bool sl_tighter = side=="BUY" ? (desired_sl>sl+point*0.5) : (sl<=0.0 || desired_sl<sl-point*0.5);
      bool tp_missing = tp<=0.0 && desired_tp>0.0;
      if((sl_tighter || tp_missing) && TradeRequestAvailable())
      {
         MarkTradeRequest();
         if(trade.PositionModify(ticket,desired_sl,desired_tp)) changed=true;
      }
   }
   return changed;
}
...
void ManageBasket()
{
   ...
   // Absolute priority: once a scout has earned profit-lock eligibility, no pending-order
   // deletion, freeze wait, telemetry action or legacy cleanup may delay protection.
   if(urgent_scout_exit_pending)
   {
      ContinueUrgentScoutExit();
      SavePersistentState();
      return;
   }
   if(!close_pending && CountOurPositions() == 1)
   {
      string priority_side = BasketSideText();
      if(priority_side == "BUY" || priority_side == "SELL")
      {
         if(basket_started_at == 0)
         {
            ...
            basket_side = priority_side;
            campaign_start_side = priority_side;
            campaign_current_side = priority_side;
            campaign_triggered_at = basket_started_at;
            campaign_phase = CAMPAIGN_OCO_CANCELLING;
         }
         campaign_current_side = priority_side;
         if(MaintainProvisionalProfitLock())
         {
            SavePersistentState();
            return;
         }
   ...
   if(InpBlockOnLegacyOpenPosition && CountLegacyEvePositions() > 0)
   {
      ...
      pending_sync_state = "CLOSE OLD EVE POSITION BEFORE V3.02";
      last_event = "v4.00 is isolated and will not overlap any recognised older EVE position";
      CancelPendingOrders();
      return;
   }
   ...
   if(positions == 0)
   {
      ...
      engine_state = "DIRECTIONAL TICK-BURST SCOUT ENGINE";
   ...
   bank_candidate = true;
   bank_candidate_reason = positions >= 2 ? "Confirmed positions share one broker-side basket SL" : "Directional scout uses dynamic profit lock and momentum-stall banking";
   ...
   // Any mixed direction is an execution safety event. v4.00 never flips or hedges inside a campaign.
   ...
   if(campaign_phase == CAMPAIGN_OCO_CANCELLING)
   {
      ...
      {
         campaign_phase = CAMPAIGN_ACTIVE;
         pending_sync_state = "SCOUT PROOF CONFIRMED";
         last_event = "Second " + campaign_current_side + " position confirmed re-acceleration; shared-SL ladder management is active";
         SendEvent("confirmed", last_event);
      }
      else
      {
         engine_state = "DIRECTIONAL SCOUT ACTIVE";
   ...
   campaign_phase = CAMPAIGN_ACTIVE;
   engine_state = "INDIVIDUAL SL/TP LADDER";

   engine_state = "RAPID LADDER - INDIVIDUAL SL/TP";
...
void MaintainMovingStraddle()
{
   ...
   MqlTick tick;
   if(!SymbolInfoTick(trade_symbol, tick) || !have_scan || last_scan.atr <= 0.0)
   {
      engine_state = "TICK ENGINE WARMING";
      pending_sync_state = "COLLECTING 30 SECONDS OF LIVE TICKS";
      CancelPendingOrders();
      return;
   }

   // velocity_1s and velocity_3s are already measured in ATR-per-second units.
   // Do not multiply them by ATR again.
   double quiet_limit = MathMax(0.0001, InpBurstResetQuietVelocityATR);
   bool quiet_now = MathAbs(last_scan.velocity_1s) <= quiet_limit &&
                    MathAbs(last_scan.velocity_3s) <= quiet_limit;
   if(!burst_reset_ready && quiet_now)
   {
      burst_reset_ready = true;
      burst_arm_reason = "QUIET RESET COMPLETE - READY FOR NEXT BURST";
      burst_arm_side = "NONE";
      last_event = burst_arm_reason;
   }

   if(burst_bracket_armed && InpBurstBracketLifetimeSeconds > 0 &&
      GetTickCount64() > burst_bracket_armed_ms + (ulong)InpBurstBracketLifetimeSeconds * 1000)
   {
      engine_state = "BURST EXPIRED";
      pending_sync_state = "NO TRIGGER - DISARMING AND WAITING FOR A FRESH ACCELERATION";
      last_event = pending_sync_state;
      CancelPendingOrders();
      burst_bracket_armed = false;
      burst_bracket_armed_ms = 0;
      burst_reset_ready = false;
      burst_arm_side = "NONE";
      ResetStraddleAnchor();
      return;
   }

   if(!burst_bracket_armed)
   {
      CancelPendingOrders();
      if(!burst_reset_ready)
      {
         engine_state = "WAITING FOR QUIET RESET";
         pending_sync_state = "NO NEW BRACKET UNTIL LIVE SPEED QUIETENS";
         return;
      }

      double counter_multiplier = MathMax(1.0, InpM5CounterTrendBurstMultiplier);
      double buy_multiplier = 1.0;
      double sell_multiplier = 1.0;
      if(InpUseM5SoftBias)
      {
         if(last_scan.m5_confirmation == "SELL") buy_multiplier = counter_multiplier;
         if(last_scan.m5_confirmation == "BUY") sell_multiplier = counter_multiplier;
      }

      bool buy_velocity = last_scan.velocity_1s >= InpBurstArmVelocity1ATR * buy_multiplier &&
                          last_scan.velocity_3s >= InpBurstArmVelocity3ATR * buy_multiplier;
      bool sell_velocity = last_scan.velocity_1s <= -InpBurstArmVelocity1ATR * sell_multiplier &&
                           last_scan.velocity_3s <= -InpBurstArmVelocity3ATR * sell_multiplier;
      bool buy_acceleration = last_scan.accelerating_buy &&
                              last_scan.acceleration >= InpBurstArmAcceleration;
      bool sell_acceleration = last_scan.accelerating_sell &&
                               last_scan.acceleration >= InpBurstArmAcceleration;
      bool buy_tick_expansion = last_scan.tick_rate_ratio >= InpBurstArmTickExpansion *
         (buy_multiplier > 1.0 ? 1.05 : 1.0);
      bool sell_tick_expansion = last_scan.tick_rate_ratio >= InpBurstArmTickExpansion *
         (sell_multiplier > 1.0 ? 1.05 : 1.0);

      bool buy_burst = buy_velocity && buy_acceleration && buy_tick_expansion && last_scan.micro_break_buy;
      bool sell_burst = sell_velocity && sell_acceleration && sell_tick_expansion && last_scan.micro_break_sell;

      if(!buy_burst && !sell_burst)
      {
         engine_state = "WATCHING LIVE TICKS";
         pending_sync_state = "NO DIRECTION-CONSISTENT BURST YET";
         burst_arm_reason = StringFormat("v1 %.3f / v3 %.3f ATR-s | accel x%.2f | ticks x%.2f | M5 %s",
            last_scan.velocity_1s, last_scan.velocity_3s, last_scan.acceleration,
            last_scan.tick_rate_ratio, last_scan.m5_confirmation);
         return;
      }

      // If both are technically true because of an unusual feed transition, use the current
      // one-second direction. v4.00 then arms only that directional scout stop by default.
      burst_arm_side = buy_burst && !sell_burst ? "BUY" :
                       sell_burst && !buy_burst ? "SELL" :
                       (last_scan.velocity_1s >= 0.0 ? "BUY" : "SELL");

      ulong now_ms = GetTickCount64();
      ulong lookback_ms = (ulong)MathMax(1.0, InpBurstAnchorLookbackSeconds) * 1000;
      ulong from_ms = now_ms > lookback_ms ? now_ms - lookback_ms : 0;
      ulong to_ms = now_ms > 150 ? now_ms - 150 : now_ms;
      double burst_high = 0.0;
      double burst_low = 0.0;
      int burst_ticks = 0;
      TickWindowStats(from_ms, to_ms, burst_high, burst_low, burst_ticks);
      if(burst_ticks < 3)
      {
         burst_high = last_scan.micro_high;
         burst_low = last_scan.micro_low;
      }

      straddle_candle_time = iTime(trade_symbol, InpExecutionTimeframe, 0);
      straddle_anchor_high = MathMax(tick.ask, burst_high > 0.0 ? burst_high : tick.ask);
      straddle_anchor_low = MathMin(tick.bid, burst_low > 0.0 ? burst_low : tick.bid);
      straddle_buy_synced_candle = 0;
      straddle_sell_synced_candle = 0;
      burst_bracket_armed = true;
      burst_reset_ready = false;
      burst_bracket_armed_ms = now_ms;
      burst_arm_reason = StringFormat("%s LIVE BURST ARMED: v1 %.3f / v3 %.3f ATR-s, accel x%.2f, ticks x%.2f, M5 %s",
         burst_arm_side, last_scan.velocity_1s, last_scan.velocity_3s,
         last_scan.acceleration, last_scan.tick_rate_ratio, last_scan.m5_confirmation);
      last_event = burst_arm_reason;
   }
   ...
   campaign_phase = CAMPAIGN_BUILDING_STRADDLE;
   engine_state = "DIRECTIONAL TICK-BURST SCOUT";

   // v4.00 is deliberately single-direction. The legacy input remains visible only so
   // older saved SET files load cleanly; it cannot enable a conflicting two-sided state.
   ENUM_ORDER_TYPE wanted = StopTypeForSide(burst_arm_side);
   double desired = burst_arm_side == "BUY" ? desired_buy : desired_sell;
   if(!FindOurPendingOrder(wanted, ticket, price, sl, tp, setup))
   {
      pending_sync_state = "PLACING " + burst_arm_side + " SCOUT STOP";
      if(!PlacePendingOrder(wanted, "INITIAL", lot, desired, atr_value, 0.0,
         "directional live-burst scout; no opposite pending order")) return;
   }

   last_bracket_buy_price = burst_arm_side == "BUY" ? PendingOrderPrice(ORDER_TYPE_BUY_STOP) : 0.0;
   last_bracket_sell_price = burst_arm_side == "SELL" ? PendingOrderPrice(ORDER_TYPE_SELL_STOP) : 0.0;
   last_bracket_refresh_at = TimeCurrent();
   campaign_phase = CAMPAIGN_STRADDLE_READY;
   pending_sync_state = burst_arm_side + " SCOUT STOP READY";
   last_event = StringFormat("%s directional burst scout ready at %.2f", burst_arm_side,
      burst_arm_side == "BUY" ? last_bracket_buy_price : last_bracket_sell_price);
}


void MaintainReverseStop()
{
   // Legacy reversal hook intentionally disabled. v4.00 never flips direction inside an active campaign.
}
...
bool PlacePendingOrder(ENUM_ORDER_TYPE type, string role, double volume, double price, double atr_value, double protect_anchor_price, string reason)
{
   ...
   MarkTradeRequest();
   bool submitted = type == ORDER_TYPE_BUY_STOP
      ? trade.BuyStop(NormalizeVolume(volume), safe_price, trade_symbol, sl, tp, time_policy, expiration, comment)
      : trade.SellStop(NormalizeVolume(volume), safe_price, trade_symbol, sl, tp, time_policy, expiration, comment);
   uint code = trade.ResultRetcode();
   if((!submitted || !TradeResultAccepted(code)) && time_policy == ORDER_TIME_SPECIFIED && code == TRADE_RETCODE_INVALID_EXPIRATION)
   {
      MarkTradeRequest();
      submitted = type == ORDER_TYPE_BUY_STOP
         ? trade.BuyStop(NormalizeVolume(volume), safe_price, trade_symbol, sl, tp, ORDER_TIME_GTC, 0, comment)
         : trade.SellStop(NormalizeVolume(volume), safe_price, trade_symbol, sl, tp, ORDER_TIME_GTC, 0, comment);
      code = trade.ResultRetcode();
   }
   if(!submitted || !TradeResultAccepted(code))
   {
   ...
   SendOrderActivity("PLACED", role, type, ticket, volume, safe_price, reason + StringFormat(" | SL %.2f | TP %.2f", sl, tp));
   return true;
}
...
void TryAddRollingLeg()
{
   ...
   // All v4.00 additions come from one broker-side pending stop order ahead.
}
...
bool ResolveMixedDirectionIfNeeded()
{
   ...
   engine_state = "MIXED DIRECTION SAFETY";
   pending_sync_state = "CANCELLING ORDERS AND CLOSING THE FULL BASKET";
   BankBasket("MIXED BUY/SELL POSITIONS DETECTED - NO REVERSAL OR HEDGE CAMPAIGN", BasketProfit());
   return true;
}
...
{
   string json = StringFormat("{\"id\":\"%I64u-%s\",\"basketId\":\"%I64d\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"3.02\",\"strategy\":\"DETERMINISTIC_SCOUT_LADDER\",\"magic\":\"%I64u\",\"action\":\"%s\",\"side\":\"%s\",\"ticket\":\"%I64u\",\"positionId\":\"%I64u\",\"volume\":%.2f,\"price\":%.5f,\"netProfit\":%.2f,\"reason\":\"%s\",\"dealTime\":%I64d}", deal_ticket, JsonEscape(action), basket_id, AccountInfoInteger(ACCOUNT_LOGIN), JsonEscape(trade_symbol), InpMagicNumber, JsonEscape(action), JsonEscape(side), deal_ticket, position_id, volume, price, net, JsonEscape(reason), (long)TimeCurrent() * 1000);
...
void SendOrderActivity(string action, string role, ENUM_ORDER_TYPE type, ulong ticket, double volume, double price, string reason)
{
   ...
   string json = StringFormat("{\"id\":\"%I64d-%I64u-%s\",\"basketId\":\"%I64d\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"3.02\",\"strategy\":\"DETERMINISTIC_SCOUT_LADDER\",\"magic\":\"%I64u\",\"action\":\"%s\",\"role\":\"%s\",\"orderType\":\"%s\",\"ticket\":\"%I64u\",\"volume\":%.2f,\"price\":%.5f,\"reason\":\"%s\",\"at\":%I64d}", (long)TimeCurrent(), ticket, JsonEscape(action), basket_id, AccountInfoInteger(ACCOUNT_LOGIN), JsonEscape(trade_symbol), InpMagicNumber, JsonEscape(action), JsonEscape(role), order_type, ticket, volume, price, JsonEscape(reason), (long)TimeCurrent() * 1000);
   QueueJson("/api/ea/order", json);
}

void SendBankDecision(string reason, double profit, int same_score, int opposite_score)
{
   string json = StringFormat("{\"id\":\"%I64d-%I64d\",\"basketId\":\"%I64d\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"3.02\",\"strategy\":\"DETERMINISTIC_SCOUT_LADDER\",\"magic\":\"%I64u\",\"side\":\"%s\",\"basketProfit\":%.2f,\"peakBasketProfit\":%.2f,\"newestTicket\":\"%I64u\",\"newestProfit\":%.2f,\"newestPeak\":%.2f,\"sameScore\":%d,\"oppositeScore\":%d,\"momentumState\":\"%s\",\"reason\":\"%s\",\"at\":%I64d}", (long)TimeCurrent(), basket_id, basket_id, AccountInfoInteger(ACCOUNT_LOGIN), JsonEscape(trade_symbol), InpMagicNumber, JsonEscape(BasketSideText()), profit, peak_basket_profit, newest_ticket, newest_leg_current_profit, newest_leg_peak_profit, same_score, opposite_score, have_scan ? JsonEscape(last_scan.momentum_state) : "UNKNOWN", JsonEscape(reason), (long)TimeCurrent() * 1000);
...
void ContinuePendingClose()
{
   ...
   // v4.00 quarantines the campaign first: pending entries are cancelled before
...
void FinaliseBasketClose(string fallback_reason)
{
   ...
   string json = StringFormat(
      "{\"id\":\"%I64d-%I64u\",\"account\":\"%I64d\",\"symbol\":\"%s\",\"version\":\"3.02\",\"strategy\":\"DETERMINISTIC_SCOUT_LADDER\",\"magic\":\"%I64u\",\"side\":\"%s\",\"startSide\":\"%s\",\"finalSide\":\"%s\",\"buyLegs\":%d,\"sellLegs\":%d,\"reversalCount\":%d,\"volume\":%.2f,\"lotPerLeg\":%.2f,\"maxTotalLotsUsed\":%.2f,\"positionsOpened\":%d,\"maxConcurrentPositions\":%d,\"entryTime\":%I64d,\"exitTime\":%I64d,\"entryPrice\":%.5f,\"exitPrice\":%.5f,\"entryScore\":%d,\"oppositeScore\":%d,\"entryRegime\":\"%s\",\"entryState\":\"%s\",\"entryReason\":\"%s\",\"exitReason\":\"%s\",\"netProfit\":%.2f,\"peakBasketProfit\":%.2f,\"profitGiveback\":%.2f,\"mfe\":%.2f,\"mae\":%.2f,\"durationSeconds\":%d,\"closeAttempts\":%d,\"closeTriggerProfit\":%.2f,\"newestTicket\":\"%I64u\",\"newestProfitAtBank\":%.2f,\"newestPeak\":%.2f,\"reversalTriggered\":%s,\"status\":\"CLOSED\"}",
   ...
   last_campaign_finished_candle = 0;
   burst_reset_ready = false;
   burst_bracket_armed = false;
   burst_bracket_armed_ms = 0;
   burst_arm_side = "NONE";
...
void ResetBasketState()
{
   ...
   urgent_scout_exit_ticket = 0;
   urgent_scout_last_request_ms = 0;
   urgent_scout_exit_reason = "";
   scout_stall_started_ms = 0;
   confirmation_armed_ms = 0;
   scout_proof_state = "WAITING FOR DIRECTIONAL BURST";
   shared_basket_stop = 0.0;
   shared_stop_state = "NOT ACTIVE";
   shared_stop_synced_positions = 0;
   execution_integrity_breach = false;
   ...
   ResetStraddleAnchor();
   campaign_phase = CAMPAIGN_FLAT;
   supervisor_state = supervisor_recovery_latched ? SUPERVISOR_RECOVERY : SUPERVISOR_IDLE;
   ClearCloseState();
}
...
{
   MqlTick tick;
   if(!SymbolInfoTick(trade_symbol,tick) || atr_value<=0.0) return false;
   double distance=PendingEntryDistancePrice();
   double legal=BrokerStopsDistancePrice()+PendingAdaptiveBufferPrice();
   if(type==ORDER_TYPE_BUY_STOP) entry=RoundPendingPrice(type,MathMax(requested_price,tick.ask+distance));
   else entry=RoundPendingPrice(type,MathMin(requested_price,tick.bid-distance));
   double sl_distance=MathMax(atr_value*MathMax(0.10,InpStopLossATR),legal);
   double tp_distance=MathMax(atr_value*MathMax(0.10,InpTakeProfitATR),legal);
   sl=NormalizePrice(type==ORDER_TYPE_BUY_STOP?entry-sl_distance:entry+sl_distance);
   tp=InpTakeProfitATR>0.0?NormalizePrice(type==ORDER_TYPE_BUY_STOP?entry+tp_distance:entry-tp_distance):0.0;
   return entry>0.0 && sl>0.0 && (type==ORDER_TYPE_BUY_STOP?sl<entry:sl>entry);
}

bool PendingOrderWrongSide(ENUM_ORDER_TYPE type, double existing_price)
{
   MqlTick tick;
   if(!SymbolInfoTick(trade_symbol, tick)) return false;
   double tick_size = SymbolInfoDouble(trade_symbol, SYMBOL_TRADE_TICK_SIZE);
   if(tick_size <= 0.0) tick_size = SymbolInfoDouble(trade_symbol, SYMBOL_POINT);
   double tolerance = MathMax(tick_size * 0.25, SymbolInfoDouble(trade_symbol, SYMBOL_POINT) * 0.25);
   if(type == ORDER_TYPE_BUY_STOP) return existing_price <= tick.ask + tolerance;
   if(type == ORDER_TYPE_SELL_STOP) return existing_price >= tick.bid - tolerance;
   return false;
}

bool PendingOrderInsideFreeze(ENUM_ORDER_TYPE type, double existing_price)
{
   // A wrong-side order is not "inside freeze". It is an execution fault and must
   // enter recovery immediately. The old one-sided formula treated every crossed
   // SELL STOP as permanently frozen because Bid-price became negative.
   if(PendingOrderWrongSide(type, existing_price)) return false;
...
bool ModifyPendingOrder(ulong ticket, ENUM_ORDER_TYPE type, string role, double desired_price, double atr_value, string reason)
{
   ...
   ENUM_ORDER_TYPE_TIME time_policy = PendingTimePolicy(role, expiration);
   MarkTradeRequest();
   bool submitted = trade.OrderModify(ticket, safe_price, sl, tp, time_policy, expiration, 0.0);
   uint code = trade.ResultRetcode();
   if((!submitted || !TradeResultAccepted(code)) && time_policy == ORDER_TIME_SPECIFIED && code == TRADE_RETCODE_INVALID_EXPIRATION)
   {
      MarkTradeRequest();
      submitted = trade.OrderModify(ticket, safe_price, sl, tp, ORDER_TIME_GTC, 0, 0.0);
      code = trade.ResultRetcode();
   }
   if(!submitted || !TradeResultAccepted(code))
   {
...
bool SynchronizeSharedBasketStop()
{
   ...
   if(status == -1)
   {
      ...
      shared_stop_state = reason;
      BankBasket("INDIVIDUAL SL/TP LADDER LEVEL REACHED", BasketProfit());
      return false;
   }
   if(status == 0)
   {
      shared_stop_state = reason == "" ? "WAITING TO CALCULATE SHARED SL" : reason;
      pending_sync_state = "WAITING TO ARM INDIVIDUAL SL/TP LADDER";
      return false;
   }
...
void SendHeartbeat()
{
   ...
   string json = StringFormat(
      ...
      JsonEscape(engine_state), JsonEscape(SupervisorStateText()), supervisor_recovery_latched ? "true" : "false", JsonEscape(supervisor_recovery_reason), JsonEscape(pending_sync_state), JsonEscape(CampaignPhaseText()), JsonEscape(campaign_start_side), JsonEscape(campaign_current_side),
...
bool ExecuteCommand(string action)
{
   ...
   else if(action == "REBUILD_BRACKET")
   {
      ...
      burst_arm_side = "NONE";
      ResetStraddleAnchor();
      last_event = "Burst watcher reset; waiting for a fresh acceleration";
   }
...
bool PostJson(string endpoint, string json)
{
   ...
   {
      PrintFormat("EVE Momentum v4.00 POST %s failed. HTTP=%d MQL=%d response=%s", endpoint, status, GetLastError(), CharArrayToString(result));
      return false;
   }
   return true;
}
...
void RecoverPersistentState()
{
   ...
   supervisor_recovery_latched = GVGet("supervisorrecovery", 0.0) > 0.5;
   supervisor_recovery_ticket = (ulong)GVGet("supervisorticket", 0.0);
   if(supervisor_recovery_latched)
   {
      supervisor_state = SUPERVISOR_RECOVERY;
      supervisor_recovery_reason = "RECOVERED EXECUTION RECOVERY LATCH - RECONCILING LIVE MT5 STATE";
   }
   ...
   if(CountOurPositions() > 0)
   {
      ...
      UpdateNewestCanary();
      last_event = close_pending ? "Recovered campaign with full-basket close pending" : "Recovered v4.00 price-triggered ladder campaign";
   }
   else
   {
      basket_started_at = 0;
      ...
      if(CountOurPendingOrders() > 0)
      {
         ...
         CancelPendingOrders();
         last_event = "Recovered flat EA; clearing old pending orders before live tick warmup";
      }
      else
      {
         last_event = "Recovered flat EA; collecting live ticks for the next genuine burst";
      }
   }
...
void SavePersistentState()
{
   ...
   GVSet("supervisorrecovery", supervisor_recovery_latched ? 1.0 : 0.0);
   GVSet("supervisorticket", (double)supervisor_recovery_ticket);
...
{
   return StringFormat("EMB302_%I64d_%I64u_", AccountInfoInteger(ACCOUNT_LOGIN), InpMagicNumber);
}
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX+"TITLE", 10, 18, "EVE MOMENTUM BURST v4.00 - DETERMINISTIC STATE MACHINE", 12, clrWhite);
...
void UpdatePanel()
{
   ...
   string live = have_scan ? StringFormat("SUP %s | v1 %.3f | v3 %.3f | spread %.1f/%.1f | protected SL %.2f", SupervisorStateText(), last_scan.velocity_1s, last_scan.velocity_3s, CurrentSpreadPoints(), MedianSpreadPoints(), BasketProtectedStop()) : "Live tick engine warming";
