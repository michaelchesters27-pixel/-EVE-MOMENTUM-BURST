...
input string InpOrderComment                   = "EVE-MOMENTUM-V2.04";
...
input int    InpTradeRequestCooldownMs           = 700;
input int    InpOrderDeleteTimeoutSeconds         = 5;
input int    InpPendingSafetyBufferPoints         = 18;
input int    InpPendingInvalidPriceBackoffSeconds = 2;
...
input int    InpBracketMaximumAgeSeconds        = 15;
input int    InpBracketArmScore                 = 4;
...
input int    InpCampaignLockSeconds             = 5;
input int    InpPostCampaignCooldownSeconds     = 15;
input int    InpReversalArmDelaySeconds         = 8;
input double InpReversalArmMinimumProfitMoney   = 0.20;

input group "Rolling momentum ladder"
input int    InpAddScore                        = 6;
input double InpAddSpacingATR                   = 0.10;
input int    InpAddCooldownSeconds              = 4;
input double InpAddOnlyAfterBasketProfit        = 0.20;
input bool   InpNeverAddToLosingBasket          = true;
input double InpNewestLegProfitBeforeAddMoney   = 0.03;
...
datetime pending_retry_after = 0;
...
datetime http_queue_retry_after = 0;
...
int OnInit()
{
   ...
   if(trade_symbol == "")
   {
      Print("EVE Momentum v2.04: could not resolve a trading symbol.");
      return INIT_FAILED;
   }
   if(!SymbolSelect(trade_symbol, true))
   {
      Print("EVE Momentum v2.04: cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE Momentum v2.04: indicator handle creation failed. Error ", GetLastError());
      return INIT_FAILED;
   }
...
void OnTimer()
{
   ...
   else if(now - last_poll_at >= MathMax(1, InpCommandPollSeconds))
   {
      PollRailway();
      last_poll_at = now;
   }
   ...
   else if(scan_dirty && have_scan && now - last_telemetry_at >= MathMax(1, InpTelemetrySeconds))
   {
      SendScan(last_scan);
      scan_dirty = false;
...
{
   if(trans.type == TRADE_TRANSACTION_ORDER_DELETE && delete_wait_ticket > 0 && trans.order == delete_wait_ticket)
   {
      delete_wait_ticket = 0;
      delete_wait_started_ms = 0;
      pending_sync_state = "DELETE CONFIRMED";
   }
   ...
   if(entry == DEAL_ENTRY_IN || entry == DEAL_ENTRY_INOUT)
   {
      ...
      if(first_leg)
      {
         ...
         campaign_invalidation_price = CampaignInvalidationFromEntry(execution_side, price);
         bool entry_bracket_still_exists = CountPendingByRole("ENTRY-BRACKET") > 0;
         campaign_phase = (order_role == "ENTRY-BRACKET" || entry_bracket_still_exists) ? CAMPAIGN_OCO_CANCELLING : CAMPAIGN_ACTIVE;
         ...
         basket_entry_reason = campaign_phase == CAMPAIGN_OCO_CANCELLING ? "OCO MOVING STRADDLE BREAKOUT" : "MOMENTUM CAMPAIGN ENTRY";
         baskets_today++;
         pending_sync_state = campaign_phase == CAMPAIGN_OCO_CANCELLING ? "OCO CANCEL REQUIRED" : "CAMPAIGN ACTIVE";
      }
      else if(order_role == "REVERSAL" && execution_side != campaign_current_side)
      {
         campaign_reversal_target = execution_side;
         campaign_phase = CAMPAIGN_REVERSING;
         campaign_reversal_count++;
         reversal_triggered = true;
         pending_sync_state = "REVERSAL TRANSITION";
         last_event = StringFormat("%s reversal stop triggered; old %s campaign legs will be closed before adding", execution_side, campaign_current_side);
         SendEvent("reversal", last_event);
      }
      else if((order_role == "ENTRY-BRACKET" || campaign_phase == CAMPAIGN_OCO_CANCELLING) && execution_side != campaign_start_side)
      {
         // Both entry-bracket orders crossed before the terminal could complete OCO cancellation.
         // Retain the first-triggered campaign side and close this accidental second side.
         campaign_phase = CAMPAIGN_OCO_CANCELLING;
         pending_sync_state = "OCO CONFLICT - CLEANING";
         last_event = StringFormat("OCO conflict detected: first side %s retained; accidental %s leg will be closed", campaign_start_side, execution_side);
         SendEvent("oco-conflict", last_event);
      }
      ...
      last_event = StringFormat("%s leg opened %.2f at %.2f; campaign %s; ladder %d/%d", execution_side, volume, price, CampaignPhaseText(), CountOurPositions(), runtime_maximum_positions);
      SavePersistentState();
      return;
   }
...
string CampaignPhaseText()
{
   ...
   if(campaign_phase == CAMPAIGN_OCO_CANCELLING) return "OCO CANCELLING";
   ...
   if(campaign_phase == CAMPAIGN_COOLDOWN) return "COOLDOWN";
   return "UNKNOWN";
}
...
string OrderRoleFromHistory(ulong order_ticket)
{
   ...
   if(StringFind(comment, "REV") >= 0) return "REVERSAL";
   if(StringFind(comment, "BRK") >= 0) return "ENTRY-BRACKET";
   return "UNKNOWN";
}
...
bool SelectedOrderMatchesRole(string role)
{
   ...
   if(role == "REVERSAL") return StringFind(comment, "REV") >= 0;
   if(role == "ENTRY-BRACKET") return StringFind(comment, "BRK") >= 0;
   return true;
}
...
void ApplyOperationalBlocks(MomentumSnapshot &scan)
{
   ...
   if(last_basket_closed_at > 0 && TimeCurrent() - last_basket_closed_at < InpPostCampaignCooldownSeconds)
   {
      scan.decision = "WAIT";
      scan.block_reason = "WAIT - completed campaign cooldown is active";
      return;
   }
   ...
   if(!runtime_testing_mode && baskets_today >= InpMaximumBasketsPerDay)
   {
      scan.decision = "WAIT";
      scan.block_reason = "WAIT - maximum baskets for the UTC day reached";
      return;
   }
   if(!runtime_testing_mode && consecutive_losses >= InpMaximumConsecutiveLosses)
   {
      scan.decision = "WAIT";
...
bool OpenMarketLeg(ENUM_POSITION_TYPE direction, double volume, double atr_value)
{
   ...
   double min_distance = BrokerStopsDistancePrice() + PendingAdaptiveBufferPrice();
   double sl_distance = MathMax(atr_value * InpStopLossATR, min_distance);
   double tp_distance = MathMax(atr_value * InpTakeProfitATR, min_distance);
   double sl = 0.0;
   double tp = 0.0;
   ...
   {
      RegisterPendingFailure(code);
      last_event = StringFormat("Momentum leg rejected: retcode %u %s", code, trade.ResultRetcodeDescription());
...
void ManageBasket()
{
   ...
   if(positions == 0)
   {
      ...
      {
         if(campaign_flat_seen_at == 0) campaign_flat_seen_at = TimeCurrent();

         // Give a triggered reversal order a very short settlement window before deciding the campaign is flat.
         if(CountPendingByRole("REVERSAL") > 0 && TimeCurrent() - campaign_flat_seen_at < 3)
         {
            engine_state = "REVERSAL SETTLING";
            pending_sync_state = "WAITING REVERSAL DEAL";
            return;
         }
         ...
         {
            campaign_phase = CAMPAIGN_CLOSING;
            pending_sync_state = "CLEARING FLAT CAMPAIGN";
            CancelPendingOrders();
            last_event = StringFormat("Campaign is flat; clearing %d pending order(s) before final report", CountOurPendingOrders());
            return;
         }
      ...
      if(TimeCurrent() < campaign_cooldown_until)
      {
         campaign_phase = CAMPAIGN_COOLDOWN;
         engine_state = "COOLDOWN";
         pending_sync_state = "POST-CAMPAIGN COOLDOWN";
         CancelPendingOrders();
         last_event = StringFormat("Post-campaign anti-whipsaw cooldown: %d seconds remain", (int)(campaign_cooldown_until - TimeCurrent()));
         return;
      }
   ...
   if(basket_started_at == 0)
   {
      ...
      campaign_phase = CAMPAIGN_OCO_CANCELLING;
      campaign_invalidation_price = CampaignInvalidationFromOpenPositions(campaign_current_side);
   }

   // If both bracket sides crossed, retain the original first-triggered side.
   if(ResolveMixedDirectionIfNeeded()) return;

   if(campaign_phase == CAMPAIGN_OCO_CANCELLING)
   {
      engine_state = "OCO SYNC";
      if(CountPendingByRole("ENTRY-BRACKET") > 0)
      {
         pending_sync_state = "CANCELLING OPPOSITE ENTRY";
         CancelPendingByRole("ENTRY-BRACKET");
         return;
      }
      ...
      pending_sync_state = "OCO CONFIRMED";
      last_event = StringFormat("OCO confirmed: %s triggered first; opposite entry order is gone", campaign_current_side);
      SendEvent("oco", last_event);
      SavePersistentState();
   }

   if(campaign_phase == CAMPAIGN_REVERSING)
   {
      engine_state = "REVERSING";
      if(ResolveMixedDirectionIfNeeded()) return;
      if(campaign_reversal_target != "NONE" && BasketSideText() == campaign_reversal_target)
      {
         campaign_current_side = campaign_reversal_target;
         basket_side = campaign_current_side;
         campaign_reversal_target = "NONE";
         campaign_triggered_at = TimeCurrent();
         campaign_trigger_price = AverageBasketEntry();
         campaign_invalidation_price = CampaignInvalidationFromOpenPositions(campaign_current_side);
         campaign_phase = CAMPAIGN_ACTIVE;
         pending_sync_state = "REVERSAL CONFIRMED";
         last_event = "Reversal confirmed; only " + campaign_current_side + " campaign legs remain";
         SendEvent("reversal", last_event);
         SavePersistentState();
      }
      else
   ...
   string current_side = BasketSideText();
   campaign_current_side = current_side;
   basket_side = current_side;
   ...
   {
      bank_candidate = true;
      bank_candidate_reason = canary_negative ? "Newest position crossed negative after the ladder developed" : "Newest position gave back most of its peak while momentum weakened";
   ...
   {
      // Bank the current campaign first. A new straddle is allowed only after the close and cooldown.
      // This avoids opening an opposite position inside ordinary M1 noise.
      bank_candidate = true;
      bank_candidate_reason = "Opposite live acceleration became dominant";
      BankBasket("MOMENTUM EXHAUSTION - OPPOSITE PRESSURE " + opposite, profit);
      return;
   }

   TrailAllPositions();

   if(TimeCurrent() - campaign_triggered_at >= MathMax(1, InpCampaignLockSeconds))
   {
      MaintainReverseStop();
      TryAddRollingLeg();
   }
   ...
   {
      pending_sync_state = "CAMPAIGN LOCK";
   }
...
void MaintainMovingStraddle()
{
   ...
   if(InpUseSessionFilter && !InsideTradingSessionUTC()) { pending_sync_state = "CLEARING"; CancelPendingOrders(); last_event = "Outside configured UTC session"; return; }
   if(!runtime_testing_mode && baskets_today >= InpMaximumBasketsPerDay) { pending_sync_state = "CLEARING"; CancelPendingOrders(); last_event = "Daily basket limit reached"; return; }
   if(!runtime_testing_mode && consecutive_losses >= InpMaximumConsecutiveLosses) { pending_sync_state = "CLEARING"; CancelPendingOrders(); last_event = "Consecutive loss limit reached"; return; }
   if(!runtime_testing_mode)
   {
      ...
      if(daily_pnl <= -day_loss_limit) { pending_sync_state = "CLEARING"; CancelPendingOrders(); last_event = "Daily loss limit reached"; return; }
      if(daily_pnl >= day_profit_target) { pending_sync_state = "CLEARING"; CancelPendingOrders(); last_event = "Daily profit target reached"; return; }
   }

   int best_score = MathMax(last_scan.buy_score, last_scan.sell_score);
   if(best_score < InpBracketArmScore || last_scan.regime == "CHAOTIC")
   {
      pending_sync_state = "CLEARING";
      CancelPendingOrders();
      last_event = "Waiting for enough live movement to arm the OCO straddle";
      return;
   }
   ...
   double distance = MathMax(InpBracketMinimumDistancePrice, MathMax(last_scan.atr * InpBracketDistanceATR, PendingEntryDistancePrice()));
   double desired_buy = RoundPendingPrice(ORDER_TYPE_BUY_STOP, tick.ask + distance);
   double desired_sell = RoundPendingPrice(ORDER_TYPE_SELL_STOP, tick.bid - distance);
   double lot = CalculateLegLot();
   if(lot <= 0) return;

   campaign_phase = CAMPAIGN_BUILDING_STRADDLE;
   pending_sync_state = "SYNCING OCO STRADDLE";
   bool ready = SyncPendingOrderSet(true, true, desired_buy, desired_sell, lot, last_scan.atr, "ENTRY-BRACKET", "OCO moving breakout bracket");
   if(ready)
   {
      ...
      pending_sync_state = "OCO STRADDLE READY";
      last_event = StringFormat("OCO straddle ready: BUY STOP %.2f / SELL STOP %.2f; first trigger wins", last_bracket_buy_price, last_bracket_sell_price);
   }
}

void MaintainReverseStop()
{
   if(campaign_phase != CAMPAIGN_ACTIVE || !remote_autonomous || local_paused || emergency_stopped || close_pending || CountOurPositions() <= 0)
   {
      if(campaign_phase != CAMPAIGN_ACTIVE)
         CancelPendingByRole("REVERSAL");
      return;
   }

   if(CountPendingByRole("ENTRY-BRACKET") > 0)
   {
      pending_sync_state = "OCO ENTRY CLEANUP";
      CancelPendingByRole("ENTRY-BRACKET");
      return;
   }

   if(TimeCurrent() - campaign_triggered_at < MathMax(1, InpReversalArmDelaySeconds))
   {
      pending_sync_state = "REVERSAL ARM DELAY";
      CancelPendingByRole("REVERSAL");
      return;
   }

   if(BasketProfit() < InpReversalArmMinimumProfitMoney)
   {
      pending_sync_state = "REVERSAL HELD - CAMPAIGN NOT PROTECTED";
      CancelPendingByRole("REVERSAL");
      return;
   }
   ...
   if(campaign_invalidation_price <= 0)
      campaign_invalidation_price = CampaignInvalidationFromOpenPositions(side);
   if(campaign_invalidation_price <= 0) return;

   MqlTick tick;
   if(!SymbolInfoTick(trade_symbol, tick)) return;
   ENUM_ORDER_TYPE type = side == "BUY" ? ORDER_TYPE_SELL_STOP : ORDER_TYPE_BUY_STOP;
   double desired = campaign_invalidation_price;

   if(type == ORDER_TYPE_BUY_STOP)
      desired = MathMax(desired, tick.ask + PendingEntryDistancePrice());
   else
      desired = MathMin(desired, tick.bid - PendingEntryDistancePrice());

   desired = RoundPendingPrice(type, desired);
   double lot = CalculateLegLot();
   if(lot <= 0) return;

   pending_sync_state = "SYNCING HARD REVERSAL";
   bool ready = type == ORDER_TYPE_BUY_STOP
      ? SyncPendingOrderSet(true, false, desired, 0.0, lot, atr_value, "REVERSAL", "fixed campaign invalidation stop")
      : SyncPendingOrderSet(false, true, 0.0, desired, lot, atr_value, "REVERSAL", "fixed campaign invalidation stop");
   if(ready)
   {
      pending_sync_state = "HARD REVERSAL READY";
      last_event = StringFormat("%s reversal stop fixed at campaign invalidation %.2f", type == ORDER_TYPE_BUY_STOP ? "BUY" : "SELL", desired);
   }
}
...
bool PlacePendingOrder(ENUM_ORDER_TYPE type, string role, double volume, double price, double atr_value, string reason)
{
   ...
   if(!BuildSafePendingLevels(type, price, atr_value, safe_price, sl, tp)) return false;
   ...
   SendOrderActivity("PLACED", role, type, ticket, volume, safe_price, reason);
   return true;
}

void TryAddRollingLeg()
{
   if(campaign_phase != CAMPAIGN_ACTIVE || local_paused || emergency_stopped || close_pending || !remote_autonomous || !have_scan) return;
   if(CountPendingByRole("ENTRY-BRACKET") > 0) return;
   ...
   if(!filling_initial)
   {
      if(InpNeverAddToLosingBasket && BasketProfit() < InpAddOnlyAfterBasketProfit) return;
      if(newest_ticket > 0 && newest_leg_current_profit < InpNewestLegProfitBeforeAddMoney) return;
   }
   ...
   if(!filling_initial && last_add_price > 0)
   {
      ...
      double required = MathMax(last_scan.atr * InpAddSpacingATR, InpBracketMinimumDistancePrice * 0.40);
      if(advance < required) return;
   }
   ...
   if(OpenMarketLeg(direction, lot, last_scan.atr))
   {
      ...
      last_event = StringFormat("Rolling %s leg added at %.2f after canary progress; %d/%d positions", side, current, CountOurPositions(), runtime_maximum_positions);
   }
}
...
bool ResolveMixedDirectionIfNeeded()
{
   ...
   string keep_side = campaign_phase == CAMPAIGN_REVERSING && campaign_reversal_target != "NONE"
      ? campaign_reversal_target
      : campaign_start_side;

   if(keep_side != "BUY" && keep_side != "SELL")
      keep_side = campaign_current_side;
   if(keep_side != "BUY" && keep_side != "SELL")
      keep_side = BasketSideText();

   engine_state = campaign_phase == CAMPAIGN_REVERSING ? "REVERSAL SYNC" : "OCO CONFLICT";
   pending_sync_state = campaign_phase == CAMPAIGN_REVERSING ? "CLOSING OLD DIRECTION" : "CLOSING ACCIDENTAL SECOND SIDE";

   if(!TradeRequestAvailable())
   {
      last_event = "Mixed directions detected; waiting for the previous broker request before cleaning the campaign";
      return true;
   }
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      {
         RegisterPendingFailure(code);
         last_event = StringFormat("Campaign direction cleanup failed for %I64u: %u %s", ticket, code, trade.ResultRetcodeDescription());
      }
      else
      {
         last_event = StringFormat("%s retained; opposite %s leg %I64u is closing", keep_side, side, ticket);
         SendEvent(campaign_phase == CAMPAIGN_REVERSING ? "reversal-sync" : "oco-cleanup", last_event);
      }
      return true;
   }
...
void TrailAllPositions()
{
   ...
   if(atr_now <= 0) return;
   double point = SymbolInfoDouble(trade_symbol, SYMBOL_POINT);
   double minimum_distance = BrokerStopsDistancePrice() + PendingAdaptiveBufferPrice();
   int digits = (int)SymbolInfoInteger(trade_symbol, SYMBOL_DIGITS);
   MqlTick tick;
   if(!SymbolInfoTick(trade_symbol, tick)) return;
   ...
   // Modify at most one ticket per cycle. This prevents a burst of overlapping broker requests.
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ulong ticket = PositionGetTicket(i);
      if(ticket == 0 || !PositionSelectByTicket(ticket) || !IsOurSelectedPosition()) continue;
      ...
      MarkTradeRequest();
      bool submitted = trade.PositionModify(ticket, desired_sl, tp);
      uint code = trade.ResultRetcode();
      if(!submitted || !TradeResultAccepted(code))
      {
         RegisterPendingFailure(code);
         PrintFormat("Trail update failed for %I64u: %u %s", ticket, code, trade.ResultRetcodeDescription());
      }
      return;
   }
}
...
void ContinuePendingClose()
{
   ...
   // Pending orders are removed one by one and confirmed gone before any position close is sent.
   if(CountOurPendingOrders() > 0)
   {
      CancelPendingOrders();
      last_event = StringFormat("CLOSE PENDING - clearing pending orders; %d remain", CountOurPendingOrders());
      SavePersistentState();
      return;
   }
   ...
   if(!TradeRequestAvailable()) return;
   datetime now = TimeCurrent();
   if(last_close_attempt_at > 0 && now - last_close_attempt_at < 1) return;
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      if(!submitted || !CloseResultAccepted(code))
      {
         ...
         if(code == TRADE_RETCODE_MARKET_CLOSED)
            ...
            last_event = StringFormat("CLOSE PENDING - attempt %d failed for %I64u: %u %s", close_attempts, ticket, code, description);
      }
      else
         last_event = StringFormat("CLOSE PENDING - close accepted for %I64u; %d position(s) remain", ticket, CountOurPositions());
      SavePersistentState();
      return;
   }
}
...
void FinaliseBasketClose(string fallback_reason)
{
   ...
   campaign_cooldown_until = exit_time + MathMax(1, InpPostCampaignCooldownSeconds);
...
bool TradeRequestAvailable()
{
   ...
   if(TimeCurrent() < pending_retry_after) return false;
   if(last_trade_request_ms > 0 && now_ms - last_trade_request_ms < (ulong)MathMax(100, InpTradeRequestCooldownMs)) return false;
   return true;
}
...
void RegisterPendingFailure(uint code)
{
      ...
      code == TRADE_RETCODE_FROZEN || code == TRADE_RETCODE_TIMEOUT || code == TRADE_RETCODE_CONNECTION)
   {
      ...
      pending_retry_after = TimeCurrent() + MathMax(1, InpPendingInvalidPriceBackoffSeconds);
...
bool BuildSafePendingLevels(ENUM_ORDER_TYPE type, double requested_price, double atr_value, double &entry, double &sl, double &tp)
{
   MqlTick tick;
   if(!SymbolInfoTick(trade_symbol, tick)) return false;
   ...
   // Re-read the live quote immediately before the request and push the order farther out if Gold moved.
   MqlTick latest;
   if(SymbolInfoTick(trade_symbol, latest))
   {
      if(type == ORDER_TYPE_BUY_STOP && entry < latest.ask + distance)
         entry = RoundPendingPrice(type, latest.ask + distance);
      if(type == ORDER_TYPE_SELL_STOP && entry > latest.bid - distance)
         entry = RoundPendingPrice(type, latest.bid - distance);
   }

   double sl_distance = MathMax(atr_value * InpStopLossATR, stops_distance);
   double tp_distance = MathMax(atr_value * InpTakeProfitATR, stops_distance);
   if(type == ORDER_TYPE_BUY_STOP)
   {
...
bool ModifyPendingOrder(ulong ticket, ENUM_ORDER_TYPE type, string role, double desired_price, double atr_value, string reason)
{
   ...
   double safe_price = 0.0, sl = 0.0, tp = 0.0;
   if(!BuildSafePendingLevels(type, desired_price, atr_value, safe_price, sl, tp)) return false;
   MarkTradeRequest();
...
bool SyncPendingOrderSet(bool want_buy, bool want_sell, double desired_buy, double desired_sell, double volume, double atr_value, string role, string reason)
{
   ...
   if(want_buy)
   {
      ...
      {
         PlacePendingOrder(ORDER_TYPE_BUY_STOP, role, volume, desired_buy, atr_value, reason);
         return false;
      }
      if((MathAbs(price - desired_buy) >= refresh_distance || TimeCurrent() - setup >= InpBracketMaximumAgeSeconds) && !PendingOrderInsideFreeze(ORDER_TYPE_BUY_STOP, price))
      {
   ...
   if(want_sell)
   {
      ...
      {
         PlacePendingOrder(ORDER_TYPE_SELL_STOP, role, volume, desired_sell, atr_value, reason);
         return false;
      }
      if((MathAbs(price - desired_sell) >= refresh_distance || TimeCurrent() - setup >= InpBracketMaximumAgeSeconds) && !PendingOrderInsideFreeze(ORDER_TYPE_SELL_STOP, price))
      {
...
void PollRailway()
{
   ...
   if(incoming_version > runtime_settings_version)
   {
      ...
      last_event = StringFormat("Dashboard settings applied: test mode %s, %.2f lot, %d initial, %d max, %.2f total lots", runtime_testing_mode ? "ON" : "OFF", runtime_fixed_lot, runtime_initial_positions, runtime_maximum_positions, runtime_maximum_total_lots);
...
bool FlushOneQueuedPost()
{
   ...
   if(TimeCurrent() < http_queue_retry_after) return false;
   ...
   http_queue_retry_after = TimeCurrent() + 2;
   return false;
}
...
bool PostJson(string endpoint, string json)
{
   ...
   {
      PrintFormat("EVE Momentum v2.04 POST %s failed. HTTP=%d MQL=%d response=%s", endpoint, status, GetLastError(), CharArrayToString(result));
      return false;
   }
   return true;
}
...
void RecoverPersistentState()
{
   ...
   runtime_testing_mode = GVGet("testingmode", runtime_testing_mode ? 1.0 : 0.0) > 0.5;
   ...
   campaign_phase = (ENUM_CAMPAIGN_PHASE)(int)GVGet("campaignphase", CAMPAIGN_FLAT);
   int start_code = (int)GVGet("campaignstartside", 0.0);
   int current_code = (int)GVGet("campaigncurrentside", 0.0);
   campaign_start_side = start_code > 0 ? "BUY" : start_code < 0 ? "SELL" : "NONE";
   campaign_current_side = current_code > 0 ? "BUY" : current_code < 0 ? "SELL" : "NONE";
   campaign_triggered_at = (datetime)GVGet("campaigntriggered", 0.0);
   campaign_trigger_price = GVGet("campaigntriggerprice", 0.0);
   campaign_invalidation_price = GVGet("campaigninvalidation", 0.0);
   ...
   campaign_cooldown_until = (datetime)GVGet("campaigncooldown", 0.0);
   post_campaign_anchor_price = GVGet("campaignanchor", 0.0);

   if(CountOurPositions() > 0)
   {
      if(basket_started_at <= 0) basket_started_at = EarliestBasketPositionTime();
      if(basket_id <= 0) basket_id = ((long)basket_started_at * 1000) + (long)(InpMagicNumber % 1000);
      string live_side = BasketSideText();
      if(campaign_start_side == "NONE") campaign_start_side = live_side;
      if(campaign_current_side == "NONE") campaign_current_side = live_side;
      basket_side = campaign_current_side;
      if(campaign_triggered_at <= 0) campaign_triggered_at = basket_started_at;
      if(campaign_invalidation_price <= 0) campaign_invalidation_price = CampaignInvalidationFromOpenPositions(campaign_current_side);
      campaign_phase = CountPendingByRole("ENTRY-BRACKET") > 0 ? CAMPAIGN_OCO_CANCELLING : CAMPAIGN_ACTIVE;
      last_event = close_pending ? "Recovered active campaign with CLOSE PENDING" : "Recovered active OCO momentum campaign";
   }
   else if(CountOurPendingOrders() > 0)
   {
      // Old pending orders are cleared so v2.04 can rebuild a clean two-sided OCO bracket.
      CancelPendingOrders();
      campaign_phase = CAMPAIGN_BUILDING_STRADDLE;
   }
   else
   {
      ResetBasketState();
      campaign_phase = TimeCurrent() < campaign_cooldown_until ? CAMPAIGN_COOLDOWN : CAMPAIGN_FLAT;
   }
...
{
   return StringFormat("EMB204_%I64d_%I64u_", AccountInfoInteger(ACCOUNT_LOGIN), InpMagicNumber);
}
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX+"TITLE", 10, 18, "EVE MOMENTUM BURST v2.04 - OCO CAMPAIGN MODE", 12, clrWhite);
