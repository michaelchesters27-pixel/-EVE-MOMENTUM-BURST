...
#property strict
#property description "M1 candle straddle and unlimited stop-order ladder. Scores are analytics only. After confirmation, every open position receives one shared broker-side basket SL."
...
input ulong  InpMagicNumber                    = 2207202609;
input string InpOrderComment                   = "EVE-MOMENTUM-V2.09";

input group "One-time legacy isolation"
input bool   InpDeleteLegacyPendingOrders       = true;                // Deletes v2.08 EVE pending orders before v2.09 starts
input ulong  InpLegacyMagicNumber               = 2207202608;          // Previous v2.08 EVE magic number
...
input int    InpReversalArmDelaySeconds         = 0;                   // Legacy input; unused in v2.09
input double InpReversalArmMinimumProfitMoney   = 0.0;                 // Legacy input; unused in v2.09
...
input double InpAddOnlyAfterBasketProfit        = 0.0;                 // Legacy input; unused in v2.09
input bool   InpNeverAddToLosingBasket          = false;               // Pending ladder is price-triggered, not P/L-gated
input double InpNewestLegProfitBeforeAddMoney   = 0.0;                 // Legacy input; unused in v2.09
...
input double InpSharedSLCommissionReservePerLot = 7.00;                 // Estimated round-trip commission per 1.00 lot
...
input double InpBreakEvenTriggerATR             = 0.10;                 // Legacy input; unused in v2.09
input double InpTrailingActivationATR           = 0.16;                 // Legacy input; unused in v2.09
input double InpNewestTrailATR                  = 0.065;                // Legacy input; unused in v2.09
input double InpMiddleTrailATR                  = 0.10;                 // Legacy input; unused in v2.09
input double InpOldestTrailATR                  = 0.16;                 // Legacy input; unused in v2.09
...
// v2.09 candle, ladder and shared-stop state.
...
int OnInit()
{
   ...
   if(trade_symbol == "")
   {
      Print("EVE Momentum v2.09: could not resolve a trading symbol.");
      return INIT_FAILED;
   }
   if(!SymbolSelect(trade_symbol, true))
   {
      Print("EVE Momentum v2.09: cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE Momentum v2.09: indicator handle creation failed. Error ", GetLastError());
      return INIT_FAILED;
   }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result)
{
   ...
   if(entry == DEAL_ENTRY_IN || entry == DEAL_ENTRY_INOUT)
   {
      ...
      if(first_leg)
      {
         ...
         campaign_phase = CAMPAIGN_OCO_CANCELLING; // v2.09: first of two same-side confirmations
         basket_entry_atr = CurrentOrderAtr();
...
bool DetectExecutionIntegrityBreach(const MqlTick &tick)
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      {
         // Older BUY positions are allowed to have an SL above their own entry because
         // every position shares the same profitable basket stop. The newest BUY must
         // still have that shared SL below its own entry.
         if((same_side_positions == 1 && sl >= entry - tolerance) || (is_newest && same_side_positions >= 2 && sl >= entry - tolerance))
         {
      ...
      {
         // Older SELL positions are allowed to have an SL below their own entry. The
         // newest SELL must retain the shared stop above its own entry.
         if((same_side_positions == 1 && sl <= entry + tolerance) || (is_newest && same_side_positions >= 2 && sl <= entry + tolerance))
         {
...
void MaintainProvisionalCampaign()
{
   ...
   double spacing = LadderSpacingPrice(side);
...
void MaintainActiveLadder()
{
   ...
   PlacePendingOrder(StopTypeForSide(side), "LADDER", CalculateLegLot(), desired, CurrentOrderAtr(), base, "continuous stop-order ladder; actual fills are synchronised to one shared basket SL");
}
...
{
   // v2.09 never adds with market orders. This helper remains only for backward compatibility.
   return false;
}
...
{
   return StringFind(comment, "EVE28-") >= 0 || StringFind(comment, "EVE-MOMENTUM-V2.08") >= 0 || StringFind(comment, "EVE26-") >= 0 || StringFind(comment, "EVE-MOMENTUM-V2.06") >= 0 || StringFind(comment, "EVE-MOMENTUM-V2.07") >= 0 ||
...
bool HandleLegacyPendingCleanup()
{
   ...
   last_event = "v2.09 is deleting v2.08 pending orders before starting its own bracket";
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      delete_wait_ticket = ticket;
      delete_wait_started_ms = GetTickCount64();
      SendOrderActivity("LEGACY-CANCEL-REQUESTED", "LEGACY", type, ticket, volume, price, "v2.09 version isolation cleanup");
      return true;
   }
   return true;
}
...
void ManageBasket()
{
   ...
   if(InpBlockOnLegacyOpenPosition && CountLegacyEvePositions() > 0)
   {
      ...
      pending_sync_state = "CLOSE OLD V2.08 POSITION BEFORE V2.09";
      last_event = "v2.09 is isolated and will not overlap an open v2.08 position";
      CancelPendingOrders();
      return;
   }
...
void MaintainMovingStraddle()
{
   ...
   // v2.09 never removes both sides simply because a new candle started.
...
{
   // Legacy v2.04 hook intentionally disabled. v2.09 uses only the two-trigger
...
bool PlacePendingOrder(ENUM_ORDER_TYPE type, string role, double volume, double price, double atr_value, double protect_anchor_price, string reason)
{
   ...
   string comment = "EVE29-LAD";
   if(role == "INITIAL" || role == "ENTRY-BRACKET") comment = "EVE29-INIT";
   else if(role == "CONFIRMATION" || role == "REVERSAL") comment = "EVE29-CONF";
...
void TryAddRollingLeg()
{
   ...
   // All v2.09 additions come from broker-side pending stop orders.
}
...
void ContinuePendingClose()
{
   ...
   // v2.09 quarantines the campaign first: pending entries are cancelled before
...
void FinaliseBasketClose(string fallback_reason)
{
   ...
   campaign_cooldown_until = 0; // v2.09 restarts immediately; no strategy cooldown.
...
bool BuildSafePendingLevels(ENUM_ORDER_TYPE type, string role, double pending_volume, double requested_price, double atr_value, double protect_anchor_price, double &entry, double &sl, double &tp)
{
   ...
   // beyond the previous trigger. After the fill, this provisional protection is
   // recalculated from actual fills and copied to every open position as one shared SL.
   if(profit_lock_role)
   {
      double lock_fraction = MathMax(0.10, MathMin(0.80, InpNewestSLPreviousLegLockFraction));
   ...
   if(profit_lock_role)
   {
      ...
      // Give the pending order the same profit-aware protection that will be copied
      // across the basket after the actual fill. This protects fast multi-order bursts
      // before all older positions have finished synchronising.
...
bool PostJson(string endpoint, string json)
{
   ...
   {
      PrintFormat("EVE Momentum v2.09 POST %s failed. HTTP=%d MQL=%d response=%s", endpoint, status, GetLastError(), CharArrayToString(result));
      return false;
   }
   return true;
}
...
void RecoverPersistentState()
{
   ...
   if(CountOurPositions() > 0)
   {
      ...
      UpdateNewestCanary();
      last_event = close_pending ? "Recovered campaign with full-basket close pending" : "Recovered v2.09 price-triggered ladder campaign";
   }
   else
   {
      basket_started_at = 0;
...
{
   return StringFormat("EMB209_%I64d_%I64u_", AccountInfoInteger(ACCOUNT_LOGIN), InpMagicNumber);
}
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX+"TITLE", 10, 18, "EVE MOMENTUM BURST v2.09 - SHARED BASKET SL", 12, clrWhite);
