...
#property strict
#property description "Deterministic directional scout with strict MT5 state reconstruction, wrong-side order recovery, profit-first protection and one shared-SL ladder order ahead."
...
input ulong  InpMagicNumber                    = 2207202632;
input string InpOrderComment                   = "EVE-MOMENTUM-V3.02";

input group "One-time legacy isolation"
input bool   InpDeleteLegacyPendingOrders       = true;                // Deletes recognised older EVE pending orders before v3.02 starts
...
input bool   InpBlockOnLegacyOpenPosition       = true;                // Prevents overlapping old EVE and v3.02 campaigns
...
input bool   InpDirectionalScoutOnly            = true;                // Legacy display input; v3.02 always uses one directional scout order
...
input bool   InpOneCampaignAttemptPerCandle      = false;               // Legacy input; unused. v3.02 is tick-burst driven, not candle driven
input bool   InpPausePendingEntriesOnWideSpread  = true;                // Execution safety only; open positions keep their broker-side SL
input int    InpReversalArmDelaySeconds         = 0;                   // Legacy input; unused in v3.02
input double InpReversalArmMinimumProfitMoney   = 0.0;                 // Legacy input; unused in v3.02
...
input double InpAddOnlyAfterBasketProfit        = 0.0;                 // Legacy input; unused in v3.02
input bool   InpNeverAddToLosingBasket          = false;               // Pending ladder is price-triggered, not P/L-gated
input double InpNewestLegProfitBeforeAddMoney   = 0.0;                 // Legacy input; unused in v3.02
...
input double InpBreakEvenTriggerATR             = 0.10;                 // Legacy input; unused in v3.02
input double InpTrailingActivationATR           = 0.16;                 // Legacy input; unused in v3.02
input double InpNewestTrailATR                  = 0.065;                // Legacy input; unused in v3.02
input double InpMiddleTrailATR                  = 0.10;                 // Legacy input; unused in v3.02
input double InpOldestTrailATR                  = 0.16;                 // Legacy input; unused in v3.02
...
// v3.02 tick-burst, provisional-lock, ladder and shared-stop state.
...
int OnInit()
{
   ...
   if(trade_symbol == "")
   {
      Print("EVE Momentum v3.02: could not resolve a trading symbol.");
      return INIT_FAILED;
   }
   if(!SymbolSelect(trade_symbol, true))
   {
      Print("EVE Momentum v3.02: cannot select symbol ", trade_symbol);
      return INIT_FAILED;
   }
   ...
   {
      Print("EVE Momentum v3.02: indicator handle creation failed. Error ", GetLastError());
      return INIT_FAILED;
   }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result)
{
   ...
   if(entry == DEAL_ENTRY_IN || entry == DEAL_ENTRY_INOUT)
   {
      ...
      if(first_leg)
      {
         ...
         campaign_phase = CAMPAIGN_OCO_CANCELLING; // v3.02: protected directional scout
...
bool PlacePendingOrderExact(ENUM_ORDER_TYPE type, string role, double volume, double entry, double sl, string reason)
{
   ...
   bool submitted = type == ORDER_TYPE_BUY_STOP
      ? trade.BuyStop(NormalizeVolume(volume), entry, trade_symbol, sl, 0.0, time_policy, expiration, comment)
      : trade.SellStop(NormalizeVolume(volume), entry, trade_symbol, sl, 0.0, time_policy, expiration, comment);
   uint code = trade.ResultRetcode();
...
void MaintainProvisionalCampaign()
{
   ...
   // Position 1 protection is always first. A frozen or stale pending order must never
   // block a broker-valid profit SL or the immediate-close fallback.
   if(MaintainProvisionalProfitLock()) return;

   // v3.02 never flips into the opposite direction inside the same campaign.
   // Only one same-side confirmation stop may survive while the scout is open.
   if(DeleteOneForProvisional(side)) return;

   string spread_reason = "";
   if(!EntrySpreadSafe(spread_reason))
   {
      engine_state = "WIDE SPREAD - SCOUT SL ONLY";
      pending_sync_state = spread_reason + " - no additional position will be armed";
      CancelPendingOrders();
      MaintainScoutStallExit();
      return;
   }

   // The scout is the proven edge in the v2.11 report. Protect and bank it first.
   if(MaintainScoutStallExit()) return;

   string proof_reason = "";
   bool proof_ready = ScoutReadyForConfirmation(side, proof_reason);
   ...
   {
      bool expired = false;
      if(OrderSelect(confirmation_ticket))
      {
         datetime setup = (datetime)OrderGetInteger(ORDER_TIME_SETUP);
         expired = InpConfirmationPendingLifetimeSec > 0 && setup > 0 &&
            TimeCurrent() > setup + InpConfirmationPendingLifetimeSec;
      }

      if(!proof_ready || expired)
      {
         if(OrderSelect(confirmation_ticket))
         {
            string why = expired ? "continuation proof expired" : "live continuation proof faded: " + proof_reason;
            DeleteSelectedPending(why);
         }
         confirmation_armed_ms = 0;
         pending_sync_state = expired ? "CONFIRMATION EXPIRED - SCOUT REMAINS PROTECTED" : "SCOUT ONLY - " + proof_reason;
         return;
      }
      ...
      {
         DeleteSelectedPending("scout profit lock advanced beyond the pending confirmation SL");
         confirmation_armed_ms = 0;
         return;
      }

      if(!SynchronizeExistingPositionsToTarget(side, confirmation_sl, "CONFIRMATION PRE-ARM")) return;
      pending_sync_state = StringFormat("SCOUT PROVED - %s CONFIRMATION %.2f / SHARED SL %.2f", side, confirmation_entry, confirmation_sl);
      scout_proof_state = "SCOUT PROVED BY PROFIT + RE-ACCELERATION";
      last_event = pending_sync_state;
      return;
   }
   ...
   if(!proof_ready)
   {
      engine_state = "SCOUT ACTIVE - WAITING PROOF";
      pending_sync_state = proof_reason;
      scout_proof_state = proof_reason;
      return;
   }
   ...
   {
      pending_sync_state = "SCOUT PROVED - WAITING FOR BROKER-SAFE CONFIRMATION GEOMETRY";
      return;
   }

   // Never weaken a profit lock that the scout has already earned. Position 2 inherits
   // the stronger existing stop when that stop is still valid below/above its new entry.
   double current_protection = MostProtectiveExistingStop(side);
   if(current_protection > 0.0)
   {
      bool stronger = side == "BUY" ? current_protection > planned_sl : current_protection < planned_sl;
      if(stronger) planned_sl = current_protection;
   }

   if(!SynchronizeExistingPositionsToTarget(side, planned_sl, "CONFIRMATION PRE-ARM")) return;

   pending_sync_state = "SCOUT PROVED - PLACING ONE RE-ACCELERATION CONFIRMATION STOP";
   if(PlacePendingOrderExact(StopTypeForSide(side), "CONFIRMATION", lot, planned_entry, planned_sl,
      "Position 2 requires protected scout profit, same-direction re-acceleration and a fresh micro-break"))
   {
      confirmation_armed_ms = GetTickCount64();
      scout_proof_state = "SCOUT PROVED - CONFIRMATION ARMED";
   }
}
...
void MaintainActiveLadder()
{
   ...
   bool continuation_ready = BasketReadyForNextLadder(side, continuation_reason);
   ...
   if(FindPendingBySideAndRole(side, "LADDER", ladder_ticket, ladder_entry, ladder_sl))
   {
      ...
      {
         if(OrderSelect(ladder_ticket)) DeleteSelectedPending("ladder continuation faded: " + continuation_reason);
         pending_sync_state = "SHARED SL HOLDS - " + continuation_reason;
         return;
      }
      ...
      {
         DeleteSelectedPending("shared basket protection advanced beyond the future ladder SL");
         return;
      }

      if(!SynchronizeExistingPositionsToTarget(side, ladder_sl, "NEXT LADDER PRE-ARM")) return;
      pending_sync_state = StringFormat("%s LADDER READY - FRESH RE-ACCELERATION STOP %.2f / SHARED SL %.2f", side, ladder_entry, ladder_sl);
      return;
   }

   if(!continuation_ready)
   {
      engine_state = "CONFIRMED BASKET - WAITING RE-ACCELERATION";
      pending_sync_state = continuation_reason;
      return;
   }
   ...
   {
      pending_sync_state = "RE-ACCELERATION PROVED - WAITING SAFE NEXT LADDER GEOMETRY";
      return;
   }
   ...
   pending_sync_state = "PLACING ONE RE-ACCELERATION LADDER STOP";
   PlacePendingOrderExact(StopTypeForSide(side), "LADDER", lot, planned_entry, planned_sl,
      "one stop ahead only; basket must be profitable and live momentum must re-accelerate");
}
...
{
   // v3.02 never adds with market orders. This helper remains only for backward compatibility.
   return false;
}
...
bool HandleLegacyPendingCleanup()
{
   ...
   last_event = "v3.02 is deleting recognised older EVE pending orders before starting";
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      delete_wait_ticket = ticket;
      delete_wait_started_ms = GetTickCount64();
      SendOrderActivity("LEGACY-CANCEL-REQUESTED", "LEGACY", type, ticket, volume, price, "v3.02 version isolation cleanup");
      return true;
   }
   return true;
}
...
void ManageBasket()
{
   ...
   if(InpBlockOnLegacyOpenPosition && CountLegacyEvePositions() > 0)
   {
      ...
      last_event = "v3.02 is isolated and will not overlap any recognised older EVE position";
      CancelPendingOrders();
      return;
   }
   ...
   // Any mixed direction is an execution safety event. v3.02 never flips or hedges inside a campaign.
   ...
   // and live momentum proves a fresh same-direction re-acceleration.
...
void MaintainMovingStraddle()
{
   ...
   if(!burst_bracket_armed)
   {
      ...
      // one-second direction. v3.02 then arms only that directional scout stop by default.
   ...
   // v3.02 is deliberately single-direction. The legacy input remains visible only so
...
{
   // Legacy reversal hook intentionally disabled. v3.02 never flips direction inside an active campaign.
}
...
bool PlacePendingOrder(ENUM_ORDER_TYPE type, string role, double volume, double price, double atr_value, double protect_anchor_price, string reason)
{
   ...
   MarkTradeRequest();
   bool submitted = type == ORDER_TYPE_BUY_STOP
      ? trade.BuyStop(NormalizeVolume(volume), safe_price, trade_symbol, sl, 0.0, time_policy, expiration, comment)
      : trade.SellStop(NormalizeVolume(volume), safe_price, trade_symbol, sl, 0.0, time_policy, expiration, comment);
   uint code = trade.ResultRetcode();
   if((!submitted || !TradeResultAccepted(code)) && time_policy == ORDER_TIME_SPECIFIED && code == TRADE_RETCODE_INVALID_EXPIRATION)
   {
      MarkTradeRequest();
...
void TryAddRollingLeg()
{
   ...
   // All v3.02 additions come from one broker-side pending stop order ahead.
}
...
void ContinuePendingClose()
{
   ...
   // v3.02 quarantines the campaign first: pending entries are cancelled before
...
bool PostJson(string endpoint, string json)
{
   ...
   {
      PrintFormat("EVE Momentum v3.02 POST %s failed. HTTP=%d MQL=%d response=%s", endpoint, status, GetLastError(), CharArrayToString(result));
      return false;
   }
   return true;
}
...
void RecoverPersistentState()
{
   ...
   if(CountOurPositions() > 0)
   {
      ...
      UpdateNewestCanary();
      last_event = close_pending ? "Recovered campaign with full-basket close pending" : "Recovered v3.02 price-triggered ladder campaign";
   }
   else
   {
      basket_started_at = 0;
...
void CreatePanel()
{
   CreateLabel(PANEL_PREFIX+"TITLE", 10, 18, "EVE MOMENTUM BURST v3.02 - DETERMINISTIC STATE MACHINE", 12, clrWhite);
